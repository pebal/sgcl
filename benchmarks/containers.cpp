...
int main(int argc, char** argv) {
    ...
    auto t0 = bench::Clock::now();
    long result = 0;
    if (c == "vector_push") result = sg ? run_vector_push<sgcl::vector<long>>(n) : run_vector_push<std::vector<long>>(n);
    else if (c == "vector_iterate") result = sg ? run_vector_iterate<sgcl::vector<long>>(n) : run_vector_iterate<std::vector<long>>(n);
    else if (c == "vector_ptr") result = sg ? run_vector_ptr<sgcl::vector<sgcl::tracked_ptr<Node>>, sgcl::tracked_ptr<Node>>(n, [](long i) { auto p = sgcl::make_tracked<Node>(); p->v = i; return p; })
                                            : run_vector_ptr<std::vector<std::shared_ptr<Node>>, std::shared_ptr<Node>>(n, [](long i) { auto p = std::make_shared<Node>(); p->v = i; return p; });
    else if (c == "deque_push") result = sg ? run_deque_push<sgcl::deque<long>>(n) : run_deque_push<std::deque<long>>(n);
    else if (c == "deque_iterate") result = sg ? run_deque_iterate<sgcl::deque<long>>(n) : run_deque_iterate<std::deque<long>>(n);
    else if (c == "list_push") result = sg ? run_list_push<sgcl::list<long>>(n) : run_list_push<std::list<long>>(n);
    else if (c == "list_iterate") result = sg ? run_list_iterate<sgcl::list<long>>(n) : run_list_iterate<std::list<long>>(n);
    else if (c == "list_erase") result = sg ? run_list_erase<sgcl::list<long>>(n) : run_list_erase<std::list<long>>(n);
    else if (c == "forward_list_push") result = sg ? run_forward_list_push<sgcl::forward_list<long>>(n) : run_forward_list_push<std::forward_list<long>>(n);
    else if (c == "map_insert") result = sg ? run_map_insert<sgcl::map<long, long>>(n) : run_map_insert<std::map<long, long>>(n);
    else if (c == "map_find") result = sg ? run_map_find<sgcl::map<long, long>>(n) : run_map_find<std::map<long, long>>(n);
    else if (c == "map_iterate") result = sg ? run_map_iterate<sgcl::map<long, long>>(n) : run_map_iterate<std::map<long, long>>(n);
    else if (c == "set_insert") result = sg ? run_set_insert<sgcl::set<long>>(n) : run_set_insert<std::set<long>>(n);
    else if (c == "unordered_insert") result = sg ? run_map_insert<sgcl::unordered_map<long, long>>(n) : run_map_insert<std::unordered_map<long, long>>(n);
    else if (c == "unordered_find") result = sg ? run_map_find<sgcl::unordered_map<long, long>>(n) : run_map_find<std::unordered_map<long, long>>(n);
    else if (c == "unordered_erase") result = sg ? run_unordered_erase<sgcl::unordered_map<long, long>>(n) : run_unordered_erase<std::unordered_map<long, long>>(n);
    else if (c == "unordered_ptr") result = sg ? run_unordered_ptr<sgcl::unordered_map<long, sgcl::tracked_ptr<Node>>, sgcl::tracked_ptr<Node>>(n, [](long i) { auto p = sgcl::make_tracked<Node>(); p->v = i; return p; })
                                               : run_unordered_ptr<std::unordered_map<long, std::shared_ptr<Node>>, std::shared_ptr<Node>>(n, [](long i) { auto p = std::make_shared<Node>(); p->v = i; return p; });
    else {
