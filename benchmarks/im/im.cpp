...
int main(int argc, char** argv) {
    ...
    if (what == "vector") {
        ...
        if (variant == "sgcl") {
            vector_case<sgcl::im::vector<long>>("sgcl", n,
                [](auto& v, long i) { return v.push_back(i); },
                [](auto& v, long i) { return v[i]; },
                [](auto& v, long i, long x) { return v.set(i, x); },
                [](long n) { std::vector<long> src(n); for (long i = 0; i < n; ++i) src[i] = i; return sgcl::im::vector<long>(src.begin(), src.end()); });
            return 0;
        }
#if SGCL_HAS_IMMER
        if (variant == "immer") {
        ...
        long n = argc > 3 ? std::atol(argv[3]) : 1000000;
        if (variant == "sgcl") {
            list_case<sgcl::im::list<long>>("sgcl", n,
        ...
        if (variant == "sgcl") {
            map_case<sgcl::im::map<long, long>>("sgcl", keys,
                [](auto& m, long k, long v) { return m.insert(k, v); },
                [](auto& m, long k) { return *m.find(k); },
                [](const std::vector<long>& keys) { std::vector<std::pair<long, long>> r; r.reserve(keys.size()); for (size_t i = 0; i < keys.size(); ++i) r.emplace_back(keys[i], (long)i); return sgcl::im::map<long, long>(r.begin(), r.end()); });
            return 0;
        }
#if SGCL_HAS_IMMER
        if (variant == "immer") {
    ...
    std::fprintf(stderr, "usage: bench_im vector sgcl|immer|immer-unsafe [n], bench_im list sgcl|std [n], bench_im map sgcl|immer|immer-unsafe|std [n]%s\n", SGCL_HAS_IMMER ? "" : "   (built without immer: cmake -DSGCL_IMMER_INCLUDE=<dir>)");
    return 2;
}
