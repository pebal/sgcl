...
//   string <sgcl|std> [op=make|copy|hash|sweep] [len=10]
//   make: 2 M strings made from tokens of a text buffer, each stored in a node
//   copy: 2 M strings copied from one node to another, the sources scattered
//   hash: 2 M strings hashed, as a map key would be
...
namespace {
    ...
    double run(const char* op, size_t len) {
        ...
        if (!std::strcmp(op, "copy")) {
            ...
            auto t0 = bench::Clock::now();
            for (long i = 0; i < count; ++i) {
                targets[i]->s = nodes[(i * 7919) % count]->s;
            }
            return bench::seconds_since(t0) / count * 1e9;
        }
        if (!std::strcmp(op, "hash")) {
            std::hash<S> h;
            size_t sum = 0;
            for (long i = 0; i < count; ++i) {
                sum += h(nodes[i]->s);
            ...
            auto ns = bench::seconds_since(t0) / count * 1e9;
            return sum == 1 ? 0 : ns;
        }
...
int main(int argc, char** argv) {
    ...
    if (!bench::has_variant(variant, {"sgcl", "std"})) {
        std::fprintf(stderr, "usage: string <sgcl|std> [make|copy|hash|sweep] [len]\n");
        return 2;
    }
