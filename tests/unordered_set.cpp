...
#include "sgcl/unordered_multiset.h"
#include "sgcl/unordered_set.h"
...
namespace {
    ...
    using IntSet = sgcl::unordered_set<Int, IntHash, IntEqual>;
    using IntMultiset = sgcl::unordered_multiset<Int, IntHash, IntEqual>;
    ...
    struct Holder {
        sgcl::unordered_set<tracked_ptr<Baz>> set;
    };
...
TEST(UnorderedSet_Test, DefaultConstructorEmpty) {
    IntSet set;
...
TEST(UnorderedSet_Test, Constructors) {
    sgcl::unordered_set<int> sized(20);
    EXPECT_EQ(sized.bucket_count(), 32u);
    EXPECT_EQ(collector::get_live_object_count(), 2u);
    std::vector<int> source = {1, 2, 3, 2, 1};
    sgcl::unordered_set<int> ranged(source.begin(), source.end());
    EXPECT_EQ(ranged.size(), 3u);
    sgcl::unordered_set<int> listed = {5, 6, 5, 7};
    EXPECT_EQ(listed.size(), 3u);
    EXPECT_TRUE(listed.contains(5));
    sgcl::unordered_set<int, SeededHash> seeded({1, 2}, 16, SeededHash{3});
    EXPECT_EQ(seeded.bucket_count(), 16u);
    EXPECT_EQ(seeded.hash_function().seed, 3u);
    EXPECT_EQ(seeded.size(), 2u);
    sgcl::unordered_set deduced(source.begin(), source.end());
    static_assert(std::is_same_v<decltype(deduced), sgcl::unordered_set<int>>);
    sgcl::unordered_set deduced_list = {std::string("a"), std::string("b")};
    static_assert(std::is_same_v<decltype(deduced_list), sgcl::unordered_set<std::string>>);
    EXPECT_EQ(deduced_list.size(), 2u);
}

TEST(UnorderedSet_Test, CopyMoveAssign) {
    IntSet other = {1, 2, 3};
    IntSet copy(other);
...
TEST(UnorderedSet_Test, InsertAndEmplace) {
    sgcl::unordered_set<std::string> set;
    std::string a = "a";
...
TEST(UnorderedSet_Test, ClearAndDestructor) {
    off_frame([] {   // the iterator insert returns is a raw pointer: it must not stay in this frame
        sgcl::unordered_set<Int, SeededHash, IntEqual> set({1, 2, 3}, 0, SeededHash{8});
...
TEST(UnorderedSet_Test, IteratorsAndSwap) {
    using Set = sgcl::unordered_set<int>;
...
TEST(UnorderedSet_Test, IteratorsInUnmanagedMemory) {
    using Set = sgcl::unordered_set<tracked_ptr<Baz>>;
    Set set;
...
TEST(UnorderedSet_Test, NodeHandles) {
    IntSet set = {1, 2, 3};
...
TEST(UnorderedSet_Test, RehashReserveAndBuckets) {
    sgcl::unordered_set<int> set;
    for (int i = 0; i < 300; ++i) {
        set.insert(i * 3);
    }
...
TEST(UnorderedSet_Test, TransparentLookup) {
    using Transparent = sgcl::unordered_set<std::string, StringViewHash, StringViewEqual>;
    static_assert(FindsStringView<Transparent>);
    static_assert(!FindsStringView<sgcl::unordered_set<std::string>>);
...
TEST(UnorderedSet_Test, TrackedElements) {
...
TEST(UnorderedSet_Test, ThrowingConstructorLeavesSetUnchanged) {
    sgcl::unordered_set<Throwing, ThrowingHash> set;
    set.emplace(1);
    auto buckets = set.bucket_count();
...
TEST(UnorderedSet_Test, Equality) {
    sgcl::unordered_set<int> a, b(1024);
    for (int i = 0; i < 100; ++i) {
        a.insert(i);
        b.insert(99 - i);
    }
...
TEST(UnorderedSet_Test, StressAgainstStd) {
    sgcl::unordered_set<int> set;
...
TEST(UnorderedMultimap_Test, DuplicateKeys) {
    sgcl::unordered_multimap<int, Int> map;
...
TEST(UnorderedMultimap_Test, EraseAndExtract) {
    sgcl::unordered_multimap<int, Int, CollidingHash> map;
    for (int i = 0; i < 40; ++i) {
        map.emplace(i % 8, i);
    }
...
TEST(UnorderedMultimap_Test, RehashKeepsRunsInOrder) {
    sgcl::unordered_multimap<int, int, CollidingHash> map;
    for (int i = 0; i < 200; ++i) {
        map.emplace(i % 16, i);
    }
    ...
    sgcl::unordered_multimap<int, int, CollidingHash> copy = map;
...
TEST(UnorderedMultimap_Test, EqualityAsMultisets) {
    sgcl::unordered_multimap<int, int> a = {{1, 1}, {1, 2}, {2, 5}};
    sgcl::unordered_multimap<int, int> b = {{2, 5}, {1, 2}, {1, 1}};
    sgcl::unordered_multimap<int, int> c = {{1, 1}, {1, 1}, {2, 5}};
    sgcl::unordered_multimap<int, int> d = {{1, 1}, {1, 2}, {2, 5}, {2, 5}};
    EXPECT_TRUE(a == b);
    ...
    c.insert({1, 2});
    EXPECT_TRUE(a == c);
    sgcl::unordered_multimap<int, int> e = {{1, 1}, {1, 1}, {1, 2}};
    sgcl::unordered_multimap<int, int> f = {{1, 1}, {1, 2}, {1, 2}};
    EXPECT_FALSE(e == f);
    EXPECT_TRUE(e == e);
}

TEST(UnorderedMultimap_Test, MergeAndSwap) {
    sgcl::unordered_multimap<int, Int> a = {{1, 1}, {1, 2}};
    sgcl::unordered_map<int, Int> b = {{1, 3}, {2, 4}};
    auto it = b.find(2);
    ...
    sgcl::unordered_multimap<int, Int> c = {{1, 5}, {3, 6}};
    a.merge(std::move(c));
    EXPECT_EQ(a.size(), 6u);
    EXPECT_EQ(a.count(1), 4u);
    EXPECT_TRUE(c.empty());
    EXPECT_EQ(Int::counter, 6u);
    sgcl::unordered_multimap<int, Int> d = {{9, 9}};
    swap(a, d);
    EXPECT_EQ(a.size(), 1u);
    EXPECT_EQ(d.size(), 6u);
...
TEST(UnorderedMultimap_Test, StressAgainstStd) {
    sgcl::unordered_multimap<int, int> map;
...
TEST(UnorderedMultiset_Test, DuplicateElements) {
    IntMultiset set;
...
TEST(UnorderedMultiset_Test, EqualityAsMultisets) {
    sgcl::unordered_multiset<int> a = {1, 1, 2};
    sgcl::unordered_multiset<int> b = {2, 1, 1};
    sgcl::unordered_multiset<int> c = {1, 2, 2};
    sgcl::unordered_multiset<int> d = {1, 1, 2, 2};
    EXPECT_TRUE(a == b);
    EXPECT_FALSE(a == c);
    EXPECT_FALSE(a == d);
    EXPECT_TRUE(a != c);
...
TEST(UnorderedMultiset_Test, RehashMergeAndTransparentLookup) {
    sgcl::unordered_multiset<int, CollidingHash> set;
    for (int i = 0; i < 100; ++i) {
        set.insert(i % 10);
    }
    auto it = set.find(7);
    ...
    sgcl::unordered_multiset<int, CollidingHash> other = {7, 7, 11};
    set.merge(other);
    EXPECT_TRUE(other.empty());
    EXPECT_EQ(set.count(7), 12u);
    EXPECT_EQ(set.count(11), 1u);
    EXPECT_TRUE(equal_keys_adjacent(set));
    sgcl::unordered_set<int, CollidingHash> unique = {7, 12};
    set.merge(unique);
    EXPECT_EQ(set.count(7), 13u);
    EXPECT_EQ(set.count(12), 1u);
    EXPECT_TRUE(unique.empty());

    using Transparent = sgcl::unordered_multiset<std::string, StringViewHash, StringViewEqual>;
    static_assert(FindsStringView<Transparent>);
    static_assert(!FindsStringView<sgcl::unordered_multiset<std::string>>);
...
TEST(UnorderedMultiset_Test, IteratorsAcrossSwapMoveAndErase) {
    sgcl::unordered_multiset<int> a = {1, 1, 2, 3};
    sgcl::unordered_multiset<int> b = {9};
    auto it = a.find(2);
    a.erase(1);
    a.erase(3);
    EXPECT_EQ(*it, 2);
    ...
    sgcl::unordered_multiset<int> c(std::move(b));
