...
TEST(Channel_Test, CoroutinePipeline) {
    ...
    sgcl::task<> w = sgcl::spawn(worker(jobs, results));   // on the scheduler: to its first co_await, waiting on jobs
    sgcl::task<int> s = sgcl::spawn(summer(results));      // waiting on results
    std::thread producer([&] {
        for (int i = 0; i < 100; ++i) {
            jobs.send(make_tracked<Baz>(i));   // each send makes the worker ready; a worker thread runs it
        }
        jobs.close();
    });
    producer.join();
    w.join();
    EXPECT_EQ(s.join(), 9900);
...
TEST(Channel_Test, CoroutineSuspendedOnFullChannelAndOnClose) {
    ...
    sgcl::atomic<int> sent = {0};
    sgcl::task<> t = sgcl::spawn(sender(out, 5, sent));
    EXPECT_TRUE(soon([&] { return sent.load() == 1; }));   // one in the buffer, the second send suspended
    EXPECT_FALSE(t.done());
    EXPECT_EQ(*out.receive(), 0);     // the receive makes the coroutine ready: it sends the next and suspends again
    ...
    sgcl::task<> r = sgcl::spawn(receiver(in, state));
    ...
    sgcl::task<> t2 = sgcl::spawn(sender(full, 1, sent2));   // suspended: full
...
TEST(Channel_Test, SignalChannel) {
    ...
    sgcl::task<> w = sgcl::spawn(waiter(done, got));
...
namespace {
    ...
    sgcl::task<> sends(sgcl::channel<int>& ch, sgcl::atomic<int>& done) {
        done = co_await ch.async_send(1) ? 1 : -1;
    }

    sgcl::task<> receives(sgcl::channel<int>& ch, sgcl::atomic<int>& got) {
        auto v = co_await ch.async_receive();
        got = v ? *v : -1;
    }

    SGCL_NOINLINE void serve_a_sender(sgcl::atomic<int>& done) {
        sgcl::channel<int> ch;                           // a rendezvous on this stack
        sgcl::go(sends(ch, done));
        std::this_thread::sleep_for(50us);               // the sender registered first, usually
        EXPECT_EQ(*ch.receive(), 1);                     // served: the channel goes with the return
    }

    SGCL_NOINLINE void serve_a_receiver(sgcl::atomic<int>& got) {
        sgcl::channel<int> ch;
        sgcl::go(receives(ch, got));
...
TEST(Channel_Test, ARendezvousReleasesASenderOnlyWhenItsElementIsTaken) {
    sgcl::channel<int> ch;                                   // capacity 0
    sgcl::atomic<int> sent = {0};
    auto sender = [](sgcl::channel<int>& ch, sgcl::atomic<int>& sent, int v) -> sgcl::task<> {
        co_await ch.async_send(v);
        ++sent;
    };
    auto a = sgcl::spawn(sender(ch, sent, 1));
    auto b = sgcl::spawn(sender(ch, sent, 2));
    std::this_thread::sleep_for(10ms);                       // both waiting
    EXPECT_EQ(sent.load(), 0);
    EXPECT_TRUE(ch.receive().has_value());                   // one taken
    std::this_thread::sleep_for(10ms);
