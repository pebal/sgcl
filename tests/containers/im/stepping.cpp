...
namespace {
    ...
    class ImStepping : public testing::TestWithParam<unsigned> {
    protected:
    ...
    sgcl::im::vector<tracked_ptr<Counted>> counted(int n) {
        sgcl::im::vector<tracked_ptr<Counted>> v;
        for (int i = 0; i < n; ++i) {
...
TEST_P(ImStepping, VectorSetAfterTheFlipKeepsTheSharedLeaves) {
    ...
    off_frame([&] {                                     // the walk in a frame of its own: a pointer it leaves would root a leaf
        ...
        v = sgcl::im::vector<tracked_ptr<Counted>>();
    });
    settle(s);
...
TEST_P(ImStepping, MapInsertAfterTheFlipKeepsTheSharedEntries) {
    ...
    sgcl::im::map<int, tracked_ptr<Counted>> m;
    off_frame([&] {
        for (int i = 0; i < n; ++i) {
    ...
    off_frame([&] {
        ...
        m = sgcl::im::map<int, tracked_ptr<Counted>>();
    });
    settle(s);
    EXPECT_EQ(Counted::alive, 0);
}
