...
namespace {
    ...
    sgcl::im::map<int, int> squares(int n) {
        sgcl::im::map<int, int> m;
        for (int i = 0; i < n; ++i) {
            m = m.insert(i, i * i);
        }
        return m;
...
TEST(ImMap_Tests, ListAndRangeConstructors) {
    sgcl::im::map<std::string, int> m = {{"a", 1}, {"b", 2}, {"a", 3}};
    EXPECT_EQ(m.size(), 2u);
    ...
    sgcl::im::map p(o.begin(), o.end());   // deduced
    static_assert(std::is_same_v<decltype(p), sgcl::im::map<int, int>>);
...
TEST(ImMap_Tests, Sharing) {
    const size_t before = collector::get_live_object_count();
    sgcl::im::map<int, int> m, plus, minus;
    off_frame([&] {
        m = squares(100000);
    });
...
TEST(ImMap_Tests, Collisions) {
    sgcl::im::map<int, int, CollidingHash> m;
...
TEST(ImMap_Tests, RangeConstructorBuiltAtOnce) {
    ...
    std::map<int, int> o;
    sgcl::im::map<int, int, CollidingHash> by_inserts;
    for (auto& [k, v] : in) {
        o[k] = v;
        by_inserts = by_inserts.insert(k, v);
    }
    sgcl::im::map<int, int, CollidingHash> m(in.begin(), in.end());
    ...
    sgcl::im::set<int, CollidingHash> s(keys.begin(), keys.end());
    EXPECT_TRUE(same_set(s, std::set<int>(keys.begin(), keys.end())));
    sgcl::im::map<int, int> e(in.end(), in.end());
    EXPECT_TRUE(e.empty());
...
TEST(ImMap_Tests, EraseToEmptyCollapses) {
    ...
    EXPECT_EQ(m.begin(), m.end());
    EXPECT_EQ(collector::get_live_object_count(), before);
    EXPECT_EQ(m, (sgcl::im::map<int, int>()));
...
TEST(ImMap_Tests, TransparentLookup) {
    sgcl::im::map<sgcl::string, int> ages;
    ...
    sgcl::im::set<sgcl::string> names = {"alice", "bob"};
...
TEST(ImMap_Tests, TrackedValues) {
    const size_t before = collector::get_live_object_count();
    sgcl::im::map<int, tracked_ptr<Baz>> m, changed;
    off_frame([&] {
        for (int i = 0; i < 100; ++i) {
...
TEST(ImMap_Tests, InsideManagedObject) {
    struct Holder {
        sgcl::im::map<std::string, tracked_ptr<Baz>> by_name;
        sgcl::im::set<int> ids;
    };
    const size_t before = collector::get_live_object_count();
...
TEST(ImMap_Tests, Iteration) {
    auto m = squares(3000);
    static_assert(std::forward_iterator<sgcl::im::map<int, int>::const_iterator>);
    static_assert(std::ranges::forward_range<sgcl::im::map<int, int>>);
    std::set<int> keys;
    long sum = 0;
...
TEST(ImSet_Tests, Basics) {
    ...
    off_frame([&] {   // the raw pointers of the checks stay below this frame
        ...
        sgcl::im::set<int> l = {3, 1, 2, 3};
        EXPECT_EQ(l.size(), 3u);
        EXPECT_EQ(l, sgcl::im::set<int>({1, 2, 3}));
        EXPECT_NE(l, b);
        std::vector<int> o = {4, 5, 6};
        sgcl::im::set d(o.begin(), o.end());
        static_assert(std::is_same_v<decltype(d), sgcl::im::set<int>>);
...
TEST(ImSet_Tests, StressAgainstOracle) {
    std::mt19937 rng(5);
    sgcl::im::set<int, CollidingHash> s, kept;
...
TEST(ImMap_Tests, ReadersUnderCopyOnWrite) {
    sgcl::copy_on_write<sgcl::im::map<int, int>> current;
