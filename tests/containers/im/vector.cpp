...
namespace {
    ...
    sgcl::im::vector<int> counted(int n) {
        sgcl::im::vector<int> v;
        for (int i = 0; i < n; ++i) {
            v = v.push_back(i);
        }
        return v;
...
TEST(ImVector_Tests, Empty) {
    ...
    EXPECT_EQ(v, sgcl::im::vector<int>());
}
...
TEST(ImVector_Tests, ListAndRangeConstructors) {
    ...
    sgcl::im::vector w(o.begin(), o.end());   // deduced
    static_assert(std::is_same_v<decltype(w), sgcl::im::vector<int>>);
...
TEST(ImVector_Tests, IteratorsAndRanges) {
    auto v = counted(5000);
    static_assert(std::random_access_iterator<sgcl::im::vector<int>::const_iterator>);
    static_assert(std::ranges::random_access_range<sgcl::im::vector<int>>);
...
TEST(ImVector_Tests, TrackedElements) {
    const size_t before = collector::get_live_object_count();
    sgcl::im::vector<tracked_ptr<Baz>> v, w;
    off_frame([&] {
        for (int i = 0; i < 100; ++i) {
...
TEST(ImVector_Tests, InsideManagedObject) {
    struct Holder {
        sgcl::im::vector<tracked_ptr<Baz>> items;
        sgcl::im::vector<std::string> names;
    };
    const size_t before = collector::get_live_object_count();
...
TEST(ImVector_Tests, Strings) {
    sgcl::im::vector<std::string> v = {"a", "b"};
    auto w = v.push_back("c").set(0, std::string("z"));
    EXPECT_EQ(v[0], "a");
    EXPECT_EQ(v.size(), 2u);
    EXPECT_EQ(w[0], "z");
    EXPECT_EQ(w[2], "c");
    sgcl::im::vector<sgcl::string> s;
    for (int i = 0; i < 100; ++i) {
...
TEST(ImVector_Tests, ReadersUnderCopyOnWrite) {
    sgcl::copy_on_write<sgcl::im::vector<int>> current;
...
TEST(ImVector_Tests, ReadersUnderAtomic) {
    sgcl::atomic<tracked_ptr<sgcl::im::vector<int>>> current(make_tracked<sgcl::im::vector<int>>());
    sgcl::atomic<bool> stop = {false};
    sgcl::atomic<bool> bad = {false};
    off_frame([&] {
        std::vector<std::thread> readers;
        for (int t = 0; t < 4; ++t) {
            readers.emplace_back([&] {
                while (!stop.load(std::memory_order_relaxed)) {
                    tracked_ptr<sgcl::im::vector<int>> s = current.load();   // the version: never modified, only replaced
                    long sum = 0;
                    for (int x : *s) {
                        sum += x;
                    }
        ...
        for (int i = 0; i < 10000; ++i) {
            ...
            current.store(make_tracked<sgcl::im::vector<int>>(std::move(v)));
        }
        stop = true;
        for (auto& r : readers) {
            r.join();
