...
namespace {
    ...
    sgcl::im::list<int> counted(int n) {
        sgcl::im::list<int> l;
        for (int i = 0; i < n; ++i) {
            l = l.push_front(i);
        }
...
TEST(ImList_Tests, Empty) {
    ...
    EXPECT_EQ(l, sgcl::im::list<int>());
    static_assert(sizeof(sgcl::im::list<int>) == 2 * sizeof(void*));
}
...
TEST(ImList_Tests, RangeAndListConstructorsKeepTheOrder) {
    sgcl::im::list<int> il = {1, 2, 3};
    EXPECT_EQ(std::vector<int>(il.begin(), il.end()), (std::vector<int>{1, 2, 3}));
    std::vector<int> v = {4, 5, 6};
    sgcl::im::list<int> from_bidi(v.begin(), v.end());
    EXPECT_EQ(std::vector<int>(from_bidi.begin(), from_bidi.end()), v);
    std::forward_list<int> f = {7, 8, 9};          // forward only: through a buffer
    sgcl::im::list<int> from_fwd(f.begin(), f.end());
    EXPECT_EQ(std::vector<int>(from_fwd.begin(), from_fwd.end()), (std::vector<int>{7, 8, 9}));
    sgcl::im::list<int> from_range(v);              // a range of ints
    EXPECT_EQ(from_range, from_bidi);
    sgcl::im::list copied(from_range);              // a copy: the same chain
    EXPECT_EQ(copied, from_range);
    sgcl::im::list<std::string> words(std::vector<const char*>{"a", "b"});   // a range of what the elements are made of
    EXPECT_EQ(words.front(), "a");
    sgcl::im::list deduced(v.begin(), v.end());
    static_assert(std::is_same_v<decltype(deduced), sgcl::im::list<int>>);
}
...
TEST(ImList_Tests, TrackedElements) {
    settle();
    sgcl::im::list<tracked_ptr<Int>> l;
    off_frame([&] {
        for (int i = 0; i < 50; ++i) {
...
TEST(ImList_Tests, ALongListDiesInOneSweep) {
    ...
    const size_t before = collector::get_live_object_count();
    off_frame([&] {
        sgcl::im::list<int> l = counted(1000000);   // a million cells, dropped at once: no chain of destructors
...
TEST(ImList_Tests, ReadersUnderCopyOnWrite) {
    sgcl::copy_on_write<sgcl::im::list<int>> current;
