...
namespace {
    ...
    struct Holder {
        sgcl::unordered_map<int, tracked_ptr<Baz>> map;
    };
...
TEST(UnorderedMap_Test, DefaultConstructorEmpty) {
    sgcl::unordered_map<int, Int> map;
...
TEST(UnorderedMap_Test, BucketCountConstructor) {
    sgcl::unordered_map<int, int> map(10);
    EXPECT_EQ(collector::get_live_object_count(), 2u);   // the bucket array and the sentinel node
    EXPECT_EQ(map.bucket_count(), 16u);
    EXPECT_TRUE(map.empty());
    sgcl::unordered_map<int, int, SeededHash, SeededEqual> seeded(4, SeededHash{7}, SeededEqual{3});
    EXPECT_EQ(seeded.bucket_count(), 4u);
    EXPECT_EQ(seeded.hash_function().seed, 7u);
    EXPECT_EQ(seeded.key_eq().tag, 3);
    sgcl::unordered_map<int, int> zero(0);
    EXPECT_EQ(zero.bucket_count(), 0u);
}

TEST(UnorderedMap_Test, RangeAndInitializerListConstructors) {
    std::vector<std::pair<const std::string, int>> source = {{"a", 1}, {"b", 2}, {"c", 3}, {"a", 4}};
    sgcl::unordered_map<std::string, int> map(source.begin(), source.end());
    ...
    sgcl::unordered_map<std::string, int> list = {{"x", 10}, {"y", 20}, {"x", 30}};
    EXPECT_EQ(list.size(), 2u);
    EXPECT_EQ(list.at("x"), 10);
    EXPECT_EQ(list.at("y"), 20);

    sgcl::unordered_map<int, int, SeededHash> seeded({{1, 1}}, 32, SeededHash{5});
    EXPECT_EQ(seeded.bucket_count(), 32u);
    EXPECT_EQ(seeded.hash_function().seed, 5u);
    EXPECT_EQ(seeded.at(1), 1);

    sgcl::unordered_map deduced(source.begin(), source.end());
    static_assert(std::is_same_v<decltype(deduced), sgcl::unordered_map<std::string, int>>);
    EXPECT_EQ(deduced.size(), 3u);
}

TEST(UnorderedMap_Test, CopyConstructor) {
    sgcl::unordered_map<int, Int, SeededHash> other({{1, 10}, {2, 20}, {3, 30}}, 0, SeededHash{9});
    other.max_load_factor(0.5f);
    sgcl::unordered_map<int, Int, SeededHash> map(other);
...
TEST(UnorderedMap_Test, MoveConstructor) {
    sgcl::unordered_map<int, Int> other = {{1, 10}, {2, 20}, {3, 30}};
    auto it = other.find(2);
    sgcl::unordered_map<int, Int> map(std::move(other));
...
TEST(UnorderedMap_Test, CopyAssignment) {
    sgcl::unordered_map<int, Int> other = {{1, 10}, {2, 20}};
    sgcl::unordered_map<int, Int> map = {{5, 50}, {6, 60}, {7, 70}};
    map = other;
...
TEST(UnorderedMap_Test, MoveAssignment) {
    sgcl::unordered_map<int, Int> other = {{1, 10}, {2, 20}};
    sgcl::unordered_map<int, Int> map = {{5, 50}, {6, 60}, {7, 70}};
    map = std::move(other);
...
TEST(UnorderedMap_Test, InitializerListAssignment) {
    sgcl::unordered_map<int, Int, SeededHash> map({{5, 50}}, 0, SeededHash{4});
...
TEST(UnorderedMap_Test, InsertVariants) {
...
TEST(UnorderedMap_Test, Emplace) {
    sgcl::unordered_map<std::string, Int> map;
...
TEST(UnorderedMap_Test, TryEmplaceConstructsInPlace) {
    sgcl::unordered_map<std::string, NonMovable> map;
    ...
    sgcl::unordered_map<int, std::unique_ptr<int>> owners;
...
TEST(UnorderedMap_Test, SubscriptAndAt) {
    sgcl::unordered_map<std::string, Int> map;
    map["x"] = 42;
    EXPECT_EQ(map["x"], 42);
    EXPECT_EQ(map.size(), 1u);
...
TEST(UnorderedMap_Test, Lookup) {
    sgcl::unordered_map<int, int> map = {{1, 10}, {2, 20}, {3, 30}};
    ...
    static_assert(std::is_same_v<decltype(cfirst), sgcl::unordered_map<int, int>::const_iterator>);
}

TEST(UnorderedMap_Test, EraseByIterator) {
    sgcl::unordered_map<int, Int> map = {{1, 10}, {2, 20}, {3, 30}, {4, 40}};
    off_frame([&] {   // a raw iterator left in a frame would keep the erased node's memory
        ...
        sgcl::unordered_map<int, Int>::const_iterator cit = map.find(3);
        map.erase(cit);
        EXPECT_EQ(map.size(), 2u);
        EXPECT_EQ(Int::counter, 2u);
    });
...
TEST(UnorderedMap_Test, EraseRange) {
    sgcl::unordered_map<int, Int> map;
    for (int i = 0; i < 20; ++i) {
        map.emplace(i, i);
    }
...
TEST(UnorderedMap_Test, EraseWhileIterating) {
    sgcl::unordered_map<int, int> map;
    for (int i = 0; i < 100; ++i) {
        map.emplace(i, i);
    }
...
TEST(UnorderedMap_Test, Clear) {
    sgcl::unordered_map<int, Int, SeededHash, SeededEqual> map({{1, 1}, {2, 2}, {3, 3}}, 0, SeededHash{2}, SeededEqual{5});
...
TEST(UnorderedMap_Test, Destructor) {
    {
        sgcl::unordered_map<int, Int> map = {{1, 1}, {2, 2}};
        EXPECT_EQ(Int::counter, 2u);
    }
    EXPECT_EQ(Int::counter, 0u);
    EXPECT_EQ(collector::get_live_object_count(), 0u);
...
TEST(UnorderedMap_Test, Swap) {
    sgcl::unordered_map<int, int, SeededHash> a({{1, 10}}, 0, SeededHash{1});
    sgcl::unordered_map<int, int, SeededHash> b({{2, 20}, {3, 30}}, 64, SeededHash{2});
...
TEST(UnorderedMap_Test, IteratorsSurviveErasingOthers) {
    sgcl::unordered_map<int, int> map;
    for (int i = 0; i < 50; ++i) {
        map.emplace(i, i * 10);
    }
...
TEST(UnorderedMap_Test, IteratorConcept) {
    using Map = sgcl::unordered_map<int, int>;
...
TEST(UnorderedMap_Test, IteratorsInUnmanagedMemory) {
    using Map = sgcl::unordered_map<int, int>;
    Map map;
    for (int i = 0; i < 100; ++i) {
        map.emplace(i, i * 2);
...
TEST(UnorderedMap_Test, RawIteratorAcrossCollection) {
    sgcl::unordered_map<int, tracked_ptr<Baz>> map;
    off_frame([&] {
        for (int i = 0; i < 50; ++i) {
...
TEST(UnorderedMap_Test, NodeHandles) {
    sgcl::unordered_map<std::string, Int> map = {{"x", 10}, {"y", 20}};
    ...
    node.key() = "w";
    node.mapped() = 15;
    sgcl::unordered_map<std::string, Int> empty;
...
TEST(UnorderedMap_Test, Merge) {
    sgcl::unordered_map<int, Int> a = {{1, 1}, {2, 2}};
    sgcl::unordered_map<int, Int> b = {{2, 20}, {3, 30}, {4, 40}};
    auto it3 = b.find(3);
    ...
    sgcl::unordered_multimap<int, Int> multi = {{5, 50}, {5, 51}, {1, 100}};
    ...
    sgcl::unordered_map<int, Int> big;
...
TEST(UnorderedMap_Test, RehashAndReserve) {
    ...
    sgcl::unordered_map<int, int> fresh;
    fresh.rehash(0);
...
TEST(UnorderedMap_Test, LoadFactorAndGrowth) {
    sgcl::unordered_map<int, int> map;
    EXPECT_EQ(map.max_load_factor(), 1.0f);
    for (int i = 0; i < 8; ++i) {
        map.emplace(i, i);
...
TEST(UnorderedMap_Test, BucketInterface) {
    sgcl::unordered_map<int, int> map;
    for (int i = 0; i < 100; ++i) {
        map.emplace(i * 7, i);
    }
    ...
    sgcl::unordered_map<int, int>::const_local_iterator clit = lit;
...
TEST(UnorderedMap_Test, TransparentLookup) {
    using Transparent = sgcl::unordered_map<std::string, int, StringViewHash, StringViewEqual>;
    using Opaque = sgcl::unordered_map<std::string, int>;
    ...
    using CountedTransparent = sgcl::unordered_map<CountedKey, int, CountedKeyHash, CountedKeyEqual>;
    using CountedOpaque = sgcl::unordered_map<CountedKey, int, OpaqueKeyHash>;
...
TEST(UnorderedMap_Test, EagerDestruction) {
    sgcl::unordered_map<Int, Int, IntHash, IntEqual> map;
    for (int i = 0; i < 10; ++i) {
        map.emplace(i, i);
    }
    ...
    map = {{20, 20}};
    EXPECT_EQ(Int::counter, 2u);
    sgcl::unordered_map<Int, Int, IntHash, IntEqual> other = {{30, 30}, {31, 31}};
    map = other;
...
TEST(UnorderedMap_Test, MappedTrackedPtr) {
    sgcl::unordered_map<int, tracked_ptr<Baz>> map;
    off_frame([&] {
        for (int i = 0; i < 3; ++i) {
...
TEST(UnorderedMap_Test, MapInsideManagedObject) {
...
TEST(UnorderedMap_Test, ThrowingConstructorLeavesMapUnchanged) {
    sgcl::unordered_map<int, Throwing> map;
    map.try_emplace(1, 1);
    auto buckets = map.bucket_count();
    ...
    sgcl::unordered_map<int, int, ThrowingHash> hashed = {{1, 1}, {2, 2}};
    ...
    sgcl::unordered_map<int, int, ThrowingHash> fresh;
    EXPECT_THROW(fresh.insert({13, 13}), std::runtime_error);
    EXPECT_TRUE(fresh.empty());
    EXPECT_EQ(fresh.bucket_count(), 0u);
}

TEST(UnorderedMap_Test, Equality) {
    sgcl::unordered_map<int, int> a;
    ...
    b.emplace(100, 100);
    EXPECT_FALSE(a == b);
    sgcl::unordered_map<int, int> empty1, empty2;
    EXPECT_TRUE(empty1 == empty2);
    EXPECT_FALSE(empty1 == a);
}

TEST(UnorderedMap_Test, EraseIf) {
    sgcl::unordered_map<int, Int> map;
    for (int i = 0; i < 100; ++i) {
        map.emplace(i, i);
    }
...
TEST(UnorderedMap_Test, StringKeys) {
    sgcl::unordered_map<std::string, std::string> map;
    ...
    sgcl::unordered_map<std::string, std::string> copy = map;
    EXPECT_TRUE(copy == map);
    copy["key-1"] += "!";
    EXPECT_FALSE(copy == map);
}

TEST(UnorderedMap_Test, StressAgainstStd) {
    sgcl::unordered_map<int, int> map;
...
TEST(UnorderedMap_Test, ARangeOfPairsCopiesEachKeyOnce) {
    ...
    sgcl::unordered_map<CountedString, int, CountedStringHash> map(src.begin(), src.end());
    EXPECT_EQ(map.size(), 3u);
    ...
    sgcl::unordered_multimap<CountedString, int, CountedStringHash> multi(src.begin(), src.end());
...
namespace {
    ...
    template<int>
    struct HandleHolder {
        sgcl::unordered_map<int, Int>::node_type node;
    };
    ...
    void drop_a_handle_in_a_managed_object(bool holder_first) {
        off_frame([&] {
            ...
            sgcl::unordered_map<int, Int> map = {{1, 10}, {2, 20}};
            if (!holder_first) {
                holder = make_tracked<HandleHolder<Tag>>();
            }
...
TEST(UnorderedMap_Test, ANodeHandleDyingInASweepDestroysItsElementOnce) {
    collector::clear_stack();
    collector::force_collect(true);
