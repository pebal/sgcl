...
namespace {
    static_assert(std::bidirectional_iterator<sgcl::map<int, int>::iterator>);
    static_assert(std::bidirectional_iterator<sgcl::map<int, int>::const_iterator>);
    static_assert(std::bidirectional_iterator<sgcl::map<std::string, std::string>::reverse_iterator>);
    static_assert(std::ranges::bidirectional_range<sgcl::map<int, int>>);
    static_assert(std::convertible_to<sgcl::map<int, int>::iterator, sgcl::map<int, int>::const_iterator>);
    static_assert(!std::convertible_to<sgcl::map<int, int>::const_iterator, sgcl::map<int, int>::iterator>);
    static_assert(!std::is_copy_constructible_v<sgcl::map<int, int>::node_type>);
    static_assert(std::is_move_constructible_v<sgcl::map<int, int>::node_type>);
    // One raw node pointer: storable anywhere, not only on the stack.
    static_assert(std::is_trivially_copyable_v<sgcl::map<int, int>::iterator>);
    static_assert(std::is_trivially_copyable_v<sgcl::map<int, int>::const_iterator>);
    static_assert(std::is_trivially_copyable_v<sgcl::multimap<int, int>::iterator>);
    static_assert(sizeof(sgcl::map<int, int>::iterator) == sizeof(void*));
    ...
    static_assert(LooksUpWith<sgcl::map<std::string, int, std::less<>>, std::string_view>);
    static_assert(!LooksUpWith<sgcl::map<std::string, int>, std::string_view>);
    static_assert(LooksUpWith<sgcl::map<std::string, int>, std::string>);
    ...
    struct HandleOwnerBefore {
        sgcl::map<int, Int>::node_type handle;
    };
...
TEST(Map_Test, ConstructorComparator) {
    sgcl::map<int, int, std::greater<int>> m(std::greater<int>{});
...
TEST(Map_Test, RangeOfAnotherType) {
    ...
    sgcl::map<std::string, int> m(src.begin(), src.end());
    EXPECT_EQ(keys_of(m), (std::vector<std::string>{"a", "b", "c"}));
    EXPECT_EQ(m["a"], 1);
    EXPECT_TRUE(m._check());
    m.insert(src.begin(), src.end());
    EXPECT_EQ(m.size(), 3u);
    sgcl::multimap<std::string, int> mm(src.begin(), src.end());
    ...
    sgcl::map<FromInt, int> from_ints(ints.begin(), ints.end());
...
TEST(Map_Test, InitializerList) {
    sgcl::map<std::string, int> m = {{"b", 2}, {"a", 1}, {"c", 3}, {"a", 4}};
    EXPECT_EQ(m.size(), 3u);
    EXPECT_EQ(keys_of(m), (std::vector<std::string>{"a", "b", "c"}));
    EXPECT_EQ(m["a"], 1);
    sgcl::map<std::string, int, std::greater<>> g({{"b", 2}, {"a", 1}}, std::greater<>{});
    EXPECT_EQ(keys_of(g), (std::vector<std::string>{"b", "a"}));
}

TEST(Map_Test, CopyConstructor) {
    sgcl::map<int, Int> other = {{1, 1}, {2, 2}, {3, 3}};
    sgcl::map<int, Int> m(other);
...
TEST(Map_Test, CopyOfALargeTree) {
    ...
    sgcl::map<int, Int> other;
    sgcl::multimap<int, Int> others;
    std::mt19937 rng(777);
    ...
    sgcl::multimap<int, Int> ms = others;
    ...
    sgcl::map<int, Int> assigned = {{-1, -1}, {-2, -2}};
    assigned = other;
    ...
    using Throwing = sgcl::map<int, ThrowingCopy>;
    Throwing source;
    off_frame([&] {
        for (int i = 0; i < 100; ++i) {
...
TEST(Map_Test, MoveConstructor) {
    ...
    sgcl::map<int, Int> m(std::move(other));
    EXPECT_EQ(collector::get_live_object_count(), 4u);
...
TEST(Map_Test, MoveAssignment) {
    sgcl::map<int, Int> other = {{4, 4}, {5, 5}, {6, 6}};
...
TEST(Map_Test, Iteration) {
    sgcl::map<int, int> m = {{5, 50}, {1, 10}, {3, 30}, {4, 40}, {2, 20}};
    ...
    sgcl::map<int, int>::const_iterator cit = it;
    EXPECT_EQ(cit, it);
    EXPECT_EQ(it, cit);
...
TEST(Map_Test, Insert) {
    sgcl::map<int, std::string> m;
...
TEST(Map_Test, TryEmplaceNonCopyable) {
    sgcl::map<int, std::unique_ptr<int>> m;
...
TEST(Map_Test, TryEmplaceNonMovable) {
    sgcl::map<int, Pinned> m;
...
TEST(Map_Test, Erase) {
    ...
    off_frame([&] {
        ...
        sgcl::map<int, Int>::const_iterator cit = m.find(4);
        it = m.erase(cit);
...
TEST(Map_Test, ClearAndDestructorAreEager) {
    off_frame([] {
        sgcl::map<int, Int> m = {{1, 1}, {2, 2}, {3, 3}};
        EXPECT_EQ(Int::counter, 3u);
        m.clear();
        EXPECT_EQ(Int::counter, 0u);
        EXPECT_TRUE(m.empty());
...
TEST(Map_Test, Swap) {
    ...
    sgcl::map<int, Int> b = {{3, 3}, {4, 4}, {5, 5}};
    auto it = a.find(2);
    a.swap(b);
...
TEST(Map_Test, IteratorsSurviveOtherErasures) {
    sgcl::map<int, int> m;
    for (int i = 0; i < 100; ++i) {
        m.emplace(i, i * 10);
    }
...
TEST(Map_Test, IteratorsInAVector) {
    using iterator = sgcl::map<int, int>::iterator;
    using const_iterator = sgcl::map<int, int>::const_iterator;
    sgcl::map<int, int> m;
    for (int i = 0; i < 100; ++i) {
        m.emplace(i, i * 10);
    }
...
TEST(Map_Test, RawIteratorSurvivesCollection) {
    sgcl::map<int, Holder> m;
    off_frame([&] {
        for (int i = 0; i < 50; ++i) {
...
TEST(Map_Test, NodeHandles) {
    ...
    off_frame([&] {
        ...
        EXPECT_EQ(m.insert(m.end(), sgcl::map<int, Int>::node_type()), m.end());
...
TEST(Map_Test, NodeHandleDyingInASweep) {
...
TEST(Map_Test, Merge) {
    sgcl::map<int, Int> a = {{1, 1}, {3, 3}, {5, 5}};
    sgcl::map<int, Int> b = {{2, 2}, {3, 30}, {4, 4}};
    auto three = b.find(3);
    a.merge(b);
...
TEST(Map_Test, Lookup) {
    sgcl::map<int, int> m = {{1, 1}, {3, 3}, {5, 5}, {7, 7}};
    const auto& cm = m;
...
TEST(Map_Test, TransparentLookup) {
    sgcl::map<std::string, int, std::less<>> m = {{"apple", 1}, {"cherry", 3}, {"banana", 2}};
    const auto& cm = m;
...
TEST(Map_Test, Comparison) {
    sgcl::map<int, int> a = {{1, 1}, {2, 2}};
    sgcl::map<int, int> b = {{1, 1}, {2, 2}};
    sgcl::map<int, int> c = {{1, 1}, {2, 3}};
    sgcl::map<int, int> d = {{1, 1}, {2, 2}, {3, 3}};
    sgcl::map<int, int> e;
    EXPECT_TRUE(a == b);
    EXPECT_FALSE(a != b);
    EXPECT_TRUE(a != c);
...
TEST(Map_Test, HintedInsertion) {
    ...
    sgcl::map<int, int> g = {{1, 1}, {3, 3}, {5, 5}};
...
TEST(Map_Test, ThrowingComparatorLeavesContainerUnchanged) {
    sgcl::map<int, Int, ThrowingLess> m;
...
TEST(Map_Test, MappedTrackedPointer) {
    sgcl::map<int, Holder> m;
    off_frame([&] {
        for (int i = 1; i <= 3; ++i) {
    ...
    EXPECT_EQ(collector::get_live_object_count(), 5u);
    nh = sgcl::map<int, Holder>::node_type();
    EXPECT_EQ(collector::get_live_object_count(), 3u);
    m.clear();
    EXPECT_EQ(collector::get_live_object_count(), 1u);
}

TEST(Map_Test, ContainerInsideManagedObject) {
    off_frame([] {
...
TEST(Map_Test, StressAgainstStdMap) {
    sgcl::map<int, int> m;
    std::map<int, int> oracle;
...
TEST(Map_Test, DeductionGuides) {
    sgcl::map m = {std::pair{1, 2.0}, std::pair{2, 3.0}};
    static_assert(std::is_same_v<decltype(m), sgcl::map<int, double>>);
    sgcl::map from_range(m.begin(), m.end());
    static_assert(std::is_same_v<decltype(from_range), sgcl::map<int, double>>);
    sgcl::map greater({std::pair{1, 2}}, std::greater<int>());
    static_assert(std::is_same_v<decltype(greater), sgcl::map<int, int, std::greater<int>>>);
    EXPECT_EQ(m.size(), 2u);
