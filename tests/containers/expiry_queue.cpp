...
// Move assignment drops the entries this queue held as clear() does: the
// object an old entry kept is no longer kept, though its handle and weak
// pointer live on; the entries of the other queue come over with it
TEST(ExpiryQueue_Tests, MoveAssignmentReleasesTheOldEntries) {
    settle();
    const int before = Texture::alive.load();
    int old_calls = 0;
    int new_calls = 0;
    ...
    expiry_queue<Texture>::entry old_entry;
    off_frame([&] {
        tracked_ptr object = make_tracked<Texture>(12);
        old_entry = a.watch(object, [&](tracked_ptr<Texture>) { ++old_calls; });
        tracked_ptr other = make_tracked<Texture>(13);
        b.watch(other, [&](tracked_ptr<Texture>) { ++new_calls; });
    });
    weak_ptr<Texture> old_weak = old_entry.weak();
    a = std::move(b);
    EXPECT_EQ(a.size(), 1u);
    EXPECT_TRUE(b.empty());
    collector::clear_stack();
    settle();
    EXPECT_TRUE(old_weak.expired());                        // released by the assignment: dead with the cycle
    EXPECT_EQ(Texture::alive.load(), before + 1);           // the entry taken over keeps its object
    EXPECT_FALSE(old_entry.cancel());                       // nothing to withdraw
    EXPECT_EQ(a.drain(), 1u);
