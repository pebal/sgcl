...
namespace {
    // A visitor with one result type per alternative: what the plain visit
    // rejects and visit<R> converts
    struct Mixed {
    ...
    struct Sum {
        auto operator()(auto x, auto y) const { return x + y; }   // int + double is a double: not one type
    };

    struct SumAsDouble {
        double operator()(auto x, auto y) const { return (double)(x + y); }
    };

    struct OnlyInt {                      // not invocable on the double alternative (an int alone would take a double by conversion)
        int operator()(int x) const { return x; }
    ...
    // Whether a call is well-formed (a requires-expression tells only
    // inside a template)
    template<class F, class... Vs>
    constexpr bool can_visit = requires(F f, Vs... vs) { visit(f, vs...); };

    template<class R, class F, class... Vs>
    constexpr bool can_visit_as = requires(F f, Vs... vs) { visit<R>(f, vs...); };

    template<class T, class V>
    constexpr bool can_hold = requires(const V& v) { holds_alternative<T>(v); };
}

TEST(Variant_Tests, VisitReturnsOneTypeForEveryAlternativeOrIsGivenIt) {
    using V = variant<int, double>;
    static_assert(!can_visit<Mixed, V&>);              // int for one alternative, double for the other: ill-formed
    static_assert(can_visit_as<double, Mixed, V&>);       // the result type given: each converts to it
    static_assert(!can_visit<Sum, V&, V&>);       // over two variants: every combination
    static_assert(can_visit<SumAsDouble, V&, V&>);
    static_assert(!can_visit<int (Member::*)() const, V&>);              // not invocable on an int
    static_assert(!can_visit<OnlyInt, V&>);                                // nor, for a later alternative, on a double: false, not an error
    V v = 2;
    EXPECT_EQ(visit<double>(Mixed(), v), 2.0);
    v = 2.5;
    EXPECT_EQ(visit<double>(Mixed(), v), 2.5);
    V w = 3;
    EXPECT_EQ(visit(SumAsDouble(), v, w), 5.5);
    EXPECT_EQ(visit<int>(Sum(), v, w), 5);
    // a pointer to member: the visitor goes through std::invoke
    variant<Member> m(Member{4});
    EXPECT_EQ(visit(&Member::f, m), 4);
    EXPECT_EQ(visit<long>(&Member::f, m), 4);
    EXPECT_EQ(visit(&Member::value, m), 4);
    visit(&Member::value, m) = 5;                                          // the result a reference, as std::invoke's
    EXPECT_EQ(get<0>(m).value, 5);
}

TEST(Variant_Tests, HoldsAlternativeIsIllFormedForADuplicateAlternative) {
    static_assert(!can_hold<int, variant<int, int>>);   // as with std: not one alternative
    static_assert(!can_hold<long, variant<int, double>>);
    static_assert(can_hold<int, variant<int, double>>);
    variant<int, int> v(std::in_place_index<1>, 3);                        // the duplicates reachable by index
