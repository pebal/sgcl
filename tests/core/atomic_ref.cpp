...
TEST(AtomicRef_Test, WaitNotifyOne) {
    ...
    sgcl::atomic<bool> threadWoke{false};
...
// For every other T it is std::atomic_ref<T>, deduced as std's is; a
// tracked_ptr or a root_ptr still gets the library's own
TEST(AtomicRef_Test, AnyOtherTypeIsStdsAtomicRef) {
    int counter = 1;
    sgcl::atomic_ref a(counter);
    static_assert(std::is_same_v<decltype(a), sgcl::atomic_ref<int>>);
    EXPECT_EQ(a.fetch_add(2), 1);
    a = 7;
    EXPECT_EQ(counter, 7);
    int expected = 7;
    EXPECT_TRUE(a.compare_exchange_strong(expected, 9));
    EXPECT_EQ(counter, 9);
    sgcl::tracked_ptr<int> p;
    static_assert(std::is_same_v<decltype(sgcl::atomic_ref(p)), sgcl::atomic_ref<sgcl::tracked_ptr<int>>>);
    sgcl::root_ptr<int> r;
    static_assert(std::is_same_v<decltype(sgcl::atomic_ref(r)), sgcl::atomic_ref<sgcl::tracked_ptr<int>>>);
}
