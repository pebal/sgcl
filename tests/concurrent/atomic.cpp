...
TEST(Atomic_Test, Exchange) {
    sgcl::atomic<sgcl::tracked_ptr<int>> atomicPtr;
    sgcl::tracked_ptr sp1 = sgcl::make_tracked<int>(1);
    sgcl::tracked_ptr sp2 = sgcl::make_tracked<int>(2);
    EXPECT_EQ(atomicPtr.exchange(sp1), nullptr);   // the old value: null
    sgcl::tracked_ptr old = atomicPtr.exchange(sp2);
    EXPECT_EQ(old, sp1);
    EXPECT_EQ(*old, 1);
    EXPECT_EQ(*atomicPtr.load(), 2);
    old = atomicPtr.exchange(nullptr);
    EXPECT_EQ(old, sp2);
    EXPECT_EQ(atomicPtr.load(), nullptr);
    // the value exchanged out is held: its object lives while old does
    old = atomicPtr.exchange(sgcl::make_tracked<int>(3));
    sgcl::tracked_ptr taken = atomicPtr.exchange(nullptr);
