...
TEST(ConcurrentUnorderedMap_Test, RangeConstructorBuiltAtOnce) {
    ...
    sgcl::concurrent_unordered_map<int, int, HashMod10> few(in.begin(), in.end());
    sgcl::concurrent_unordered_map<int, int> m(in.begin(), in.end());
    sgcl::concurrent_unordered_map<int, int> by_inserts;
    ...
    sgcl::concurrent_unordered_set<int> s(std::begin(few_keys), std::end(few_keys));
    EXPECT_EQ(s.size(), 3u);
    std::vector<std::pair<int, int>> none;
    sgcl::concurrent_unordered_map<int, int> e(none.begin(), none.end());
    EXPECT_TRUE(e.empty());
...
TEST(ConcurrentUnorderedMap_Test, TransparentLookupWithStringKeys) {
    sgcl::concurrent_unordered_map<sgcl::string, int> m = {{"alice", 30}, {"bob", 40}};
    sgcl::concurrent_unordered_set<sgcl::string> s = {"alice", "bob"};
