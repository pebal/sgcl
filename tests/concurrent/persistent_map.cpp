...
namespace {
    ...
    sgcl::persistent_map<int, int> squares(int n) {
        sgcl::persistent_map<int, int> m;
        for (int i = 0; i < n; ++i) {
            m = m.insert(i, i * i);
        }
        return m;
...
TEST(PersistentMap_Test, InsertFindErase) {
    sgcl::persistent_map<int, std::string> m;
    auto a = m.insert(1, "one");
    auto b = a.insert(2, "two");
    auto c = b.insert(1, "uno");     // replaced
    auto d = c.erase(2);
    EXPECT_TRUE(m.empty());
    EXPECT_EQ(a.size(), 1u);
    EXPECT_EQ(b.size(), 2u);
    ...
    EXPECT_EQ(*b.find(2), "two");
    EXPECT_EQ(*c.find(1), "uno");
    EXPECT_EQ(*c.find(2), "two");
    EXPECT_EQ(*d.find(1), "uno");
    ...
    EXPECT_EQ(*e.find(3), "xxx");
    auto f = e.insert(sgcl::pair<const int, std::string>(4, "four"));
    EXPECT_EQ(*f.find(4), "four");
    EXPECT_EQ(f.size(), 3u);
}

TEST(PersistentMap_Test, ListAndRangeConstructors) {
    sgcl::persistent_map<std::string, int> m = {{"a", 1}, {"b", 2}, {"a", 3}};
    EXPECT_EQ(m.size(), 2u);
    EXPECT_EQ(*m.find("a"), 3);   // the last one wins, as an insert would
    std::map<int, int> o;
    for (int i = 0; i < 300; ++i) {
        o[i] = -i;
    }
    sgcl::persistent_map p(o.begin(), o.end());   // deduced
    static_assert(std::is_same_v<decltype(p), sgcl::persistent_map<int, int>>);
...
TEST(PersistentMap_Test, OldVersionUnchanged) {
    auto m = squares(1000);
    auto plus = m.insert(1000, 1);
    auto minus = m.erase(500);
    auto changed = m.insert(500, -1);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        EXPECT_EQ(*plus.find(i), i * i);
        EXPECT_EQ(*changed.find(i), i == 500 ? -1 : i * i);
        if (i != 500) {
            EXPECT_EQ(*minus.find(i), i * i);
        }
    }
    EXPECT_EQ(minus.find(500), nullptr);
    EXPECT_EQ(*plus.find(1000), 1);
    EXPECT_NE(m, changed);
    EXPECT_EQ(m, changed.insert(500, 250000));
    EXPECT_EQ(m, minus.insert(500, 250000));
    EXPECT_EQ(m, plus.erase(1000));
}

// Two versions differing in one element share all but a path
TEST(PersistentMap_Test, Sharing) {
    const size_t before = collector::get_live_object_count();
    sgcl::persistent_map<int, int> m, plus, minus;
    off_frame([&] {
        m = squares(100000);
    });
...
// persistent as the rest
TEST(PersistentMap_Test, Collisions) {
    sgcl::persistent_map<int, int, CollidingHash> m;
    ...
    EXPECT_EQ(m.find(1200), nullptr);
    EXPECT_EQ(m.find(-1), nullptr);
    auto c = m.insert(500, 7).erase(3).erase(999).erase(1100).insert(0, 0);
    EXPECT_EQ(c.size(), 1197u);
    EXPECT_EQ(*c.find(500), 7);
    EXPECT_EQ(*m.find(500), 1000);
    EXPECT_EQ(*c.find(0), 0);
...
TEST(PersistentMap_Test, RangeConstructorBuiltAtOnce) {
    ...
    // keys (the last occurrence stays, as an insert would replace) and
    ...
    std::map<int, int> o;
    sgcl::persistent_map<int, int, CollidingHash> by_inserts;
    for (auto& [k, v] : in) {
        o[k] = v;
        by_inserts = by_inserts.insert(k, v);
    }
    sgcl::persistent_map<int, int, CollidingHash> m(in.begin(), in.end());
    ...
    EXPECT_EQ(m.find(1500), nullptr);
    auto n = m.insert(1500, 1).erase(0).erase(999);   // a version over the built trie
    EXPECT_EQ(n.size(), o.size() - 1);
    EXPECT_EQ(*m.find(999), o[999]);
    std::vector<int> keys;
    for (auto& [k, v] : in) {
        keys.push_back(k);
    }
    sgcl::persistent_set<int, CollidingHash> s(keys.begin(), keys.end());
    EXPECT_TRUE(same_set(s, std::set<int>(keys.begin(), keys.end())));
    sgcl::persistent_map<int, int> e(in.end(), in.end());
    EXPECT_TRUE(e.empty());
    EXPECT_EQ(e.begin(), e.end());
}

TEST(PersistentMap_Test, EraseToEmptyCollapses) {
    const size_t before = collector::get_live_object_count();
    sgcl::persistent_map<int, int> m;
    off_frame([&] {
        m = squares(5000);
    ...
    EXPECT_EQ(m.begin(), m.end());
    EXPECT_EQ(collector::get_live_object_count(), before);
    EXPECT_EQ(m, (sgcl::persistent_map<int, int>()));
...
TEST(PersistentMap_Test, TransparentLookup) {
    sgcl::persistent_map<sgcl::string, int> ages;
    ages = ages.insert("alice", 30).insert("bob", 32);
    std::string_view view("alice");
    EXPECT_EQ(*ages.find(view), 30);
    EXPECT_EQ(*ages.find("bob"), 32);
    ...
    for (int i = 0; i < 10000; ++i) {
        sum += *ages.find("alice") + ages.at(view) + int(ages.count("bob")) + int(without.contains("alice"));
    }
    ...
    sgcl::persistent_set<sgcl::string> names = {"alice", "bob"};
...
// reaches it
TEST(PersistentMap_Test, TrackedValues) {
    const size_t before = collector::get_live_object_count();
    sgcl::persistent_map<int, tracked_ptr<Baz>> m, changed;
    off_frame([&] {
        for (int i = 0; i < 100; ++i) {
    ...
    EXPECT_GT(one, 100u);
    off_frame([&] {
        changed = m.insert(5, make_tracked<Baz>(-5));
    });
    ...
    off_frame([&] {                        // the raw pointers of the checks stay out of this frame
        EXPECT_EQ((*m.find(5))->value, 5);
        EXPECT_EQ((*changed.find(5))->value, -5);
        EXPECT_EQ(m.find(6)->get(), changed.find(6)->get());   // the same object in both
    });
    m = {};
    collector::force_collect(true);
    off_frame([&] {
        EXPECT_EQ((*changed.find(5))->value, -5);
        EXPECT_EQ((*changed.find(6))->value, 6);
    });
    changed = {};
    EXPECT_EQ(collector::get_live_object_count(), before);
}
...
TEST(PersistentMap_Test, ElementsDestroyed) {
    settle();
...
TEST(PersistentMap_Test, InsideManagedObject) {
    struct Holder {
        sgcl::persistent_map<std::string, tracked_ptr<Baz>> by_name;
        sgcl::persistent_set<int> ids;
    };
    const size_t before = collector::get_live_object_count();
    ...
    off_frame([&] {
        ...
        EXPECT_EQ((*h->by_name.find("49"))->value, 49);
...
TEST(PersistentMap_Test, Iteration) {
    auto m = squares(3000);
    static_assert(std::forward_iterator<sgcl::persistent_map<int, int>::const_iterator>);
    static_assert(std::ranges::forward_range<sgcl::persistent_map<int, int>>);
    std::set<int> keys;
    long sum = 0;
...
TEST(PersistentMap_Test, StressAgainstOracle) {
    std::mt19937 rng(11);
    sgcl::persistent_map<int, int> m, kept;
...
TEST(PersistentSet_Test, Basics) {
    ...
    off_frame([&] {   // the raw pointers of the checks stay below this frame
        ...
        sgcl::persistent_set<int> l = {3, 1, 2, 3};
        EXPECT_EQ(l.size(), 3u);
        EXPECT_EQ(l, sgcl::persistent_set<int>({1, 2, 3}));
        EXPECT_NE(l, b);
        std::vector<int> o = {4, 5, 6};
        sgcl::persistent_set d(o.begin(), o.end());
        static_assert(std::is_same_v<decltype(d), sgcl::persistent_set<int>>);
...
TEST(PersistentSet_Test, StressAgainstOracle) {
    std::mt19937 rng(5);
    sgcl::persistent_set<int, CollidingHash> s, kept;
...
TEST(PersistentMap_Test, ReadersUnderCopyOnWrite) {
    sgcl::copy_on_write<sgcl::persistent_map<int, int>> current;
