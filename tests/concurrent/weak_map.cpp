...
//------------------------------------------------------------------------------
// concurrent::weak_map, concurrent::weak_set: the weak containers shared by
...
TEST(ConcurrentWeakMap_Tests, ValuesAreFoundByTheObject) {
    concurrent::weak_map<Node, std::string> names;
...
TEST(ConcurrentWeakMap_Tests, ANullPointerHasNoEntry) {
    concurrent::weak_map<Node, int> counts;
    ...
    concurrent::weak_set<Node> objects;
...
TEST(ConcurrentWeakMap_Tests, ADeadEntryDoesNotBlockTheSlotsNextObject) {
    ...
    concurrent::weak_map<Node, int> ids;
...
TEST(ConcurrentWeakMap_Tests, AValueHoldingItsKeyKeepsTheEntryAlive) {
    settle();
    const int before = Node::alive.load();
    concurrent::weak_map<Node, tracked_ptr<Node>> self;
    off_frame([&] {
...
TEST(ConcurrentWeakMap_Tests, MapInsideManagedObject) {
    ...
    const int before = Node::alive.load();
    struct Holder {
        concurrent::weak_map<Node, tracked_ptr<Baz>> m;
    };
...
TEST(ConcurrentWeakMap_Tests, ManyThreadsInsertObjectsTheyDrop) {
    ...
    off_frame([&] {
        concurrent::queue<tracked_ptr<Node>> kept_by_threads;   // what the threads keep, handed over
        auto round = [&](int base) {
