...
TEST(ConcurrentSortedMap_Test, RangeConstructorBuiltAtOnce) {
    ...
    sgcl::concurrent_sorted_map<int, int> m(in.begin(), in.end());
    sgcl::concurrent_sorted_map<int, int> by_inserts;
    ...
    sgcl::concurrent_sorted_set<int> s = {5, 3, 5, 1, 3};
    EXPECT_EQ(s.size(), 3u);
    EXPECT_EQ(*s.begin(), 1);
    std::vector<std::pair<int, int>> none;
    sgcl::concurrent_sorted_map<int, int> e(none.begin(), none.end());
    EXPECT_TRUE(e.empty());
    EXPECT_TRUE(e.try_emplace(1, 1).second);
}

TEST(ConcurrentSortedMap_Test, InsertFindErase) {
    sgcl::concurrent_sorted_map<int, std::string> m;
    EXPECT_TRUE(m.empty());
    EXPECT_EQ(m.size(), 0u);
...
TEST(ConcurrentSortedMap_Test, TransparentLookupWithStringKeys) {
    sgcl::concurrent_sorted_map<sgcl::string, int> m = {{"alice", 30}, {"bob", 40}, {"carol", 50}};
    sgcl::concurrent_sorted_set<sgcl::string> s = {"alice", "bob"};
...
TEST(ConcurrentSortedMap_Test, Bounds) {
    sgcl::concurrent_sorted_map<int, int> m = {{10, 0}, {20, 0}, {30, 0}};
...
TEST(ConcurrentSortedMap_Test, Ordering) {
    std::vector<std::pair<const int, int>> v = {{5, 50}, {1, 10}, {4, 40}, {2, 20}, {3, 30}};
    sgcl::concurrent_sorted_map<int, int> m(v.begin(), v.end());
    EXPECT_EQ(keys_of(m), (std::vector<int>{1, 2, 3, 4, 5}));
    int sum = 0;
...
TEST(ConcurrentSortedMap_Test, ManyKeys) {
    sgcl::concurrent_sorted_map<int, int> m;
    std::mt19937 rng(7);
...
TEST(ConcurrentSortedMap_Test, EraseByIterator) {
    sgcl::concurrent_sorted_map<int, int> m = {{1, 1}, {2, 2}, {3, 3}};
    auto it = m.find(2);
...
TEST(ConcurrentSortedMap_Test, ElementsDieWithTheirNodes) {
...
TEST(ConcurrentSortedMap_Test, NodesAndObjectsReclaimed) {
    const size_t before = collector::get_live_object_count();
    sgcl::concurrent_sorted_map<int, tracked_ptr<Baz>> m;
...
TEST(ConcurrentSortedMap_Test, MapInsideManagedObject) {
    struct Holder {
        sgcl::concurrent_sorted_map<int, tracked_ptr<Baz>> m;
    };
...
TEST(ConcurrentSortedMap_Test, DisjointInsertsManyThreads) {
    const int threads = 8;
...
TEST(ConcurrentSortedMap_Test, SameKeysManyThreads) {
    const int threads = 8;
    const int n = 5000;
    sgcl::concurrent_sorted_map<int, int> m;
...
TEST(ConcurrentSortedMap_Test, ChurnManyThreads) {
    const int threads = 8;
