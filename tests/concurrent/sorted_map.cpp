...
TEST(ConcurrentSortedMap_Test, RangeConstructorBuiltAtOnce) {
    ...
    sgcl::concurrent::sorted_map<int, int> m(in.begin(), in.end());
    sgcl::concurrent::sorted_map<int, int> by_inserts;
    ...
    sgcl::concurrent::sorted_set<int> s = {5, 3, 5, 1, 3};
    EXPECT_EQ(s.size(), 3u);
    EXPECT_EQ(*s.begin(), 1);
    std::vector<std::pair<int, int>> none;
    sgcl::concurrent::sorted_map<int, int> e(none.begin(), none.end());
    EXPECT_TRUE(e.empty());
...
TEST(ConcurrentSortedMap_Test, TransparentLookupWithStringKeys) {
    sgcl::concurrent::sorted_map<sgcl::string, int> m = {{"alice", 30}, {"bob", 40}, {"carol", 50}};
    sgcl::concurrent::sorted_set<sgcl::string> s = {"alice", "bob"};
...
TEST(ConcurrentSortedMap_Test, Bounds) {
    sgcl::concurrent::sorted_map<int, int> m = {{10, 0}, {20, 0}, {30, 0}};
...
TEST(ConcurrentSortedMap_Test, Ordering) {
    ...
    sgcl::concurrent::sorted_map<int, int> m(v.begin(), v.end());
    EXPECT_EQ(keys_of(m), (std::vector<int>{1, 2, 3, 4, 5}));
    int sum = 0;
...
TEST(ConcurrentSortedMap_Test, EraseByIterator) {
    sgcl::concurrent::sorted_map<int, int> m = {{1, 1}, {2, 2}, {3, 3}};
    auto it = m.find(2);
...
TEST(ConcurrentSortedMap_Test, NodesAndObjectsReclaimed) {
    ...
    sgcl::concurrent::sorted_map<int, tracked_ptr<Baz>> m;
