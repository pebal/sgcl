...
TEST(ConcurrentCache_Test, PutGetErase) {
    using Cache = sgcl::concurrent_cache<int, std::string>;
    Cache c(10);
    EXPECT_TRUE(c.empty());
    EXPECT_EQ(c.size(), 0u);
...
TEST(ConcurrentCache_Test, CapacityRespected) {
    sgcl::concurrent_cache<int, int> c(100);
    ...
    sgcl::concurrent_cache<int, int> one(1);
    one.put(1, 1);
    one.put(2, 2);
    EXPECT_EQ(one.size(), 1u);
    EXPECT_TRUE(one.get(2));
    sgcl::concurrent_cache<int, int> none(0);   // keeps nothing
    none.put(1, 1);
...
TEST(ConcurrentCache_Test, TheReadOneStays) {
    sgcl::concurrent_cache<std::string, std::string> c(2);
    c.put("a", "1");
    c.put("b", "2");
...
TEST(ConcurrentCache_Test, RecentlyUsedSurvive) {
    ...
    const int hot = 100;
    sgcl::concurrent_cache<int, int> c(capacity);
...
TEST(ConcurrentCache_Test, TimeToLive) {
    sgcl::concurrent_cache<int, int> c(10, 30ms);
    ...
    sgcl::concurrent_cache<int, int> e(4, 30ms);   // the stale go first at an eviction
    for (int i = 0; i < 4; ++i) {
        e.put(i, i);
    }
    std::this_thread::sleep_for(50ms);
...
TEST(ConcurrentCache_Test, GetOrCompute) {
    sgcl::concurrent_cache<int, std::string> c(10);
    int computed = 0;
    ...
    EXPECT_EQ(c.size(), 1u);
    sgcl::concurrent_cache<int, int> small(2);
    for (int i = 0; i < 10; ++i) {
...
TEST(ConcurrentCache_Test, HitsAndMisses) {
    sgcl::concurrent_cache<int, int> c(10);
    EXPECT_EQ(c.hits(), 0u);
    ...
    EXPECT_EQ(c.hits(), 3u);
    sgcl::concurrent_cache<int, int> t(10, 20ms);
    t.put(1, 1);
...
TEST(ConcurrentCache_Test, TransparentLookupWithStringKeys) {
    sgcl::concurrent_cache<sgcl::string, int> c(10);
    c.put("alice", 30);
    c.put("bob", 40);
...
TEST(ConcurrentCache_Test, ValuesLiveWhileCachedAndDieWhenEvicted) {
    ...
    sgcl::concurrent_cache<int, tracked_ptr<Int>> c(50);
    off_frame([&] {
        for (int i = 0; i < 50; ++i) {
...
TEST(ConcurrentCache_Test, CacheInsideManagedObject) {
    struct Holder {
        sgcl::concurrent_cache<int, tracked_ptr<Baz>> cache{4};
    };
...
TEST(ConcurrentCache_Test, StressManyThreads) {
    ...
    const int ops = 40000;
    off_frame([&] {
        sgcl::concurrent_cache<int, tracked_ptr<Baz>> c(capacity, {}, 8);
...
TEST(ConcurrentCache_Test, APutIsNeverLostUnderAStaleGet) {
    ...
    off_frame([&] {
        sgcl::concurrent_cache<int, int> c(100000, 300us, 8);
