...
TEST(ConcurrentPriorityQueue_Test, PopsInOrder) {
    sgcl::concurrent_priority_queue<int> q;
    EXPECT_TRUE(q.empty());
    EXPECT_EQ(q.size(), 0u);
...
TEST(ConcurrentPriorityQueue_Test, ConstructorsAndTypes) {
    sgcl::concurrent_priority_queue<int> a = {3, 1, 2};
    EXPECT_EQ(drain(a), (std::vector<int>{1, 2, 3}));
    std::vector<int> v = {4, 6, 5};
    sgcl::concurrent_priority_queue<int> b(v.begin(), v.end());
    EXPECT_EQ(drain(b), (std::vector<int>{4, 5, 6}));
    sgcl::concurrent_priority_queue<std::string> s;
    s.emplace(3, 'x');
    s.emplace("abc");
    s.push("b");
    ...
    static_assert(std::is_same_v<sgcl::concurrent_priority_queue<int>::value_type, int>);
    static_assert(std::is_same_v<sgcl::concurrent_priority_queue<int>::value_compare, std::less<int>>);
}

TEST(ConcurrentPriorityQueue_Test, CustomComparator) {
    sgcl::concurrent_priority_queue<int, std::greater<int>> q = {3, 9, 1, 7};
    EXPECT_EQ(*q.try_top(), 9);
    EXPECT_EQ(drain(q), (std::vector<int>{9, 7, 3, 1}));
    auto longer = [](const std::string& a, const std::string& b) { return a.size() > b.size(); };
    sgcl::concurrent_priority_queue<std::string, decltype(longer)> l(longer);
    l.push("a");
    l.push("ccc");
    l.push("bb");
...
TEST(ConcurrentPriorityQueue_Test, DuplicatesFifo) {
    sgcl::concurrent_priority_queue<Job, ByPriority> q;
    q.push({1, 1});
    q.push({1, 2});
    q.push({0, 3});
    q.push({1, 4});
    ...
    sgcl::concurrent_priority_queue<int> same;
...
TEST(ConcurrentPriorityQueue_Test, DuplicatesFifoPerProducerManyThreads) {
    ...
    off_frame([&] {
        sgcl::concurrent_priority_queue<Job, ByPriority> q;   // three priorities, each producer's ids in order
...
TEST(ConcurrentPriorityQueue_Test, TryTopCopiesAndLeavesTheElement) {
    sgcl::concurrent_priority_queue<std::string> q = {"b", "a"};
    auto top = q.try_top();
    ASSERT_TRUE(top);
    EXPECT_EQ(*top, "a");
    EXPECT_EQ(q.size(), 2u);
...
TEST(ConcurrentPriorityQueue_Test, ObjectsHeldByTheHeapAndReleasedAtThePop) {
    ...
    sgcl::concurrent_priority_queue<tracked_ptr<Baz>, ByValue> q;
    off_frame([&] {
        for (int i = 99; i >= 0; --i) {
...
TEST(ConcurrentPriorityQueue_Test, MoveOnlyElements) {
    ...
    sgcl::concurrent_priority_queue<Payload, decltype(by_value)> q(by_value);
    q.push(Payload(3));
    q.emplace(1);
    q.push(Payload(2));
    ...
    sgcl::concurrent_priority_queue<tracked_ptr<Payload>, decltype(by_ptr)> p(by_ptr);   // or behind a tracked_ptr, one word
...
TEST(ConcurrentPriorityQueue_Test, QueueInsideManagedObject) {
    struct Holder {
        sgcl::concurrent_priority_queue<int> q;
        sgcl::concurrent_priority_queue<tracked_ptr<Baz>, ByValue> p;
    };
