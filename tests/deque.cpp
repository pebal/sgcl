...
TEST(Deque_Test, ExceptionSafety) {
    ...
    Thrower value(7);
    Thrower::remaining = 2;   // the private copy and the first pushed copy succeed, the second throws
    EXPECT_THROW(dq.insert(dq.begin() + 3, 3, value), std::runtime_error);
    EXPECT_EQ(dq.size(), 5u);
    EXPECT_EQ(values(), expected);
    EXPECT_EQ(Thrower::live, 6);
    Thrower::remaining = 1;
    EXPECT_THROW(dq.insert(dq.begin() + 1, 3, value), std::runtime_error);   // front half
    EXPECT_EQ(values(), expected);
