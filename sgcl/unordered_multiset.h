...
namespace sgcl {
    ...
    class unordered_multiset : public detail::HashTable<detail::HashSetTraits<Key, Hash, KeyEqual, false, Ptr>> {
        using Base = detail::HashTable<detail::HashSetTraits<Key, Hash, KeyEqual, false, Ptr>>;
    ...
    unordered_multiset(InputIt, InputIt, size_t = 0, Hash = Hash(), KeyEqual = KeyEqual())
        -> unordered_multiset<typename std::iterator_traits<InputIt>::value_type, Hash, KeyEqual, Ptr>;

    template<class Key, class Hash = std::hash<Key>, class KeyEqual = std::equal_to<Key>, template<class> class Ptr = tracked_ptr>
    unordered_multiset(std::initializer_list<Key>, size_t = 0, Hash = Hash(), KeyEqual = KeyEqual())
        -> unordered_multiset<Key, Hash, KeyEqual, Ptr>;

    template<class Key, class Hash, class KeyEqual, template<class> class Ptr, class Pred>
    size_t erase_if(unordered_multiset<Key, Hash, KeyEqual, Ptr>& c, Pred pred) {
