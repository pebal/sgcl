...
namespace sgcl {
    ...
    class unordered_multimap : public detail::HashTable<detail::HashMapTraits<Key, T, Hash, KeyEqual, false, Ptr>> {
        using Base = detail::HashTable<detail::HashMapTraits<Key, T, Hash, KeyEqual, false, Ptr>>;
    ...
    unordered_multimap(std::initializer_list<std::pair<Key, T>>, size_t = 0, Hash = Hash(), KeyEqual = KeyEqual())
        -> unordered_multimap<Key, T, Hash, KeyEqual, Ptr>;

    template<class Key, class T, class Hash, class KeyEqual, template<class> class Ptr, class Pred>
    size_t erase_if(unordered_multimap<Key, T, Hash, KeyEqual, Ptr>& c, Pred pred) {
