...
namespace sgcl::concurrent {
    ...
    // positions are kept a cache line apart (config::cache_line_size).
    ...
    private:
        ...
        // (config::backoff_max) 144 and 70; two and four threads within
        ...
        // push's loop: the value is taken only by the attempt that finds
        // room, so a failed one leaves it for the next
        template<class V>
        void _push(V&& value) {
            for (;;) {
                size_type pos, seq;
                if (_try_push(pos, seq, std::forward<V>(value))) {
                    return;
                }
                _wait(_cell(pos), seq);
            }
