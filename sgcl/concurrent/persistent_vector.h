...
namespace sgcl {
    namespace detail {
        // A branch of the trie of persistent_vector: 32 children, each a
    ...
    // The persistent vector of Clojure and Scala: a sequence every
    // operation of which returns a new vector and leaves the old one as it
    // was, the two sharing everything but the path that changed. A trie of
    ...
    public:
        ...
        private:
            ...
            const_iterator(const persistent_vector* vector, size_t index) noexcept
            : _vector(vector)
            , _index(index) {
            }

            const persistent_vector* _vector = nullptr;
            size_t _index = 0;
        ...
        persistent_vector() noexcept = default;

        template<std::input_iterator InputIt>
        persistent_vector(InputIt first, InputIt last) {
        ...
        : persistent_vector(ilist.begin(), ilist.end()) {
        }

        persistent_vector(const persistent_vector&) noexcept = default;
        persistent_vector(persistent_vector&&) noexcept = default;
        persistent_vector& operator=(const persistent_vector&) noexcept = default;
        persistent_vector& operator=(persistent_vector&&) noexcept = default;
        ...
        const_reference at(size_type i) const {
            if (i >= _size) {
                throw std::out_of_range("sgcl::persistent_vector::at");
            }
            return _at(i);
        }
        ...
        template<class... A>
        persistent_vector emplace_back(A&&... a) const {
        ...
        persistent_vector pop_back() const {
            ...
            auto count = _tail_count();
            if (count > 1) {
                return persistent_vector(_size - 1, _shift, _root, _copy_leaf(*_tail, count - 1));
            }
            ...
            if (last < Width) {      // that leaf was the trie's only one: the trie is empty now
                return persistent_vector(_size - 1, 0, nullptr, tail);
            }
            unique_ptr<Branch> root = _pop_leaf(_shift, *_root, last);
            if (_shift > Bits && !root->children[1]) {   // one child left: the trie loses a level
                return persistent_vector(_size - 1, _shift - Bits, static_pointer_cast<Branch>(root->children[0]), tail);
            }
            return persistent_vector(_size - 1, _shift, std::move(root), tail);
        }
    ...
    private:
        persistent_vector(size_t size, size_t shift, tracked_ptr<Branch> root, tracked_ptr<Leaf> tail) noexcept
        : _size(size)
        , _shift(shift)
        , _root(std::move(root))
        , _tail(std::move(tail)) {
        ...
        static tracked_ptr<void> _new_path(size_t level, const tracked_ptr<void>& leaf) {
            ...
            unique_ptr<Branch> branch = make_tracked<Branch>();
        ...
        static unique_ptr<Branch> _push_leaf(size_t level, const Branch& node, size_t index, const tracked_ptr<void>& leaf) {
            unique_ptr<Branch> copy = make_tracked<Branch>(node);
        ...
        void _absorb_tail() {
            ...
            if (!_root) {
                unique_ptr<Branch> root = make_tracked<Branch>();
        ...
        template<class... A>
        persistent_vector _push_back(A&&... a) const {
            persistent_vector v = *this;
            auto count = _tail_count();
            if (count == Width) {
                v._absorb_tail();
                count = 0;
        ...
        template<class U>
        persistent_vector _set(size_type i, U&& value) const {
            if (i >= _size) {
                throw std::out_of_range("sgcl::persistent_vector::set");
            }
            auto offset = _tail_offset();
            if (i >= offset) {
                return persistent_vector(_size, _shift, _root, _copy_leaf(*_tail, _tail_count(), uint32_t(i - offset), std::forward<U>(value)));
            }
            return persistent_vector(_size, _shift, _set_in(_shift, *_root, i, std::forward<U>(value)), _tail);
        }
    ...
    template<std::input_iterator InputIt>
    persistent_vector(InputIt, InputIt) -> persistent_vector<typename std::iterator_traits<InputIt>::value_type>;
}
