...
namespace sgcl::concurrent {
    ...
    // over map, so that a lookup is the map's
    ...
    public:
        ...
        using clock = sgcl::clock;   // the library's: the steady clock, or the manual clock while one is installed (a test moves the time to live on without sleeping)
    ...
    private:
        ...
        using Map = map<Key, Entry, Hash, KeyEqual>;
        ...
        struct Counters {
            struct Cell {
                ...
                unsigned char _pad[config::cache_line_size - 2 * sizeof(atomic<uint64_t>) - sizeof(atomic<tracked_ptr<Cursor>>)] = {};
            };

            atomic<uint64_t> tick = {0};
            unsigned char _pad0[config::cache_line_size - sizeof(atomic<uint64_t>)] = {};
            atomic<long> size = {0};
            unsigned char _pad1[config::cache_line_size - sizeof(atomic<long>)] = {};
            Cell cell[Stripes];
        };
