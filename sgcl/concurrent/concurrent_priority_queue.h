...
namespace sgcl {
    ...
    // (detail/backoff.h) and a park on the word after the spin, so that a
    ...
    public:
        ...
        : concurrent_priority_queue(Compare()) {
        }
        ...
        concurrent_priority_queue(InputIt first, InputIt last, const Compare& comp = Compare())
        : concurrent_priority_queue(comp) {
        ...
        : concurrent_priority_queue(ilist.begin(), ilist.end(), comp) {
        }
    ...
    private:
        ...
        // losing a wakeup, as in concurrent_bounded_queue.h)
        ...
        alignas(config::CacheLineSize) std::atomic<uint64_t> _pushed = {0};
