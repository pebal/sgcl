...
namespace sgcl::immutable::detail {
    ...
    // Clojure's PersistentHashMap uses it, persistent (immutable, every version kept): every insert and
    ...
    // word put it beyond a terabyte. `owned` marks a node a builder
    // made and may change in place (Hamt: insert_in_place): a node
    // reached from nothing but that builder's trie, since the builder's
    // freeze clears the mark before the trie is handed out; in the high
    // half of the second word, where a mark keeps the word beyond any
    // address as the counts do.
    struct HamtHead {
        ...
        uint16_t owned = 0;      // a builder's, changed in place: 1; 0 for a node any map may hold
    };
    ...
    // Made by make() only (the constructor is private), so that a node
    // is always a managed object, which the copy of its entries relies on
    // (Hamt::_copy: the links taken unshaded, the source node shaded).
    ...
    private:
        template<class> friend class Hamt;   // find_at: an iterator placed at a key
    ...
    // The trie with the operations of a map or set of im: Traits
    ...
    public:
        ...
        // the low chunk first), a key that occurs twice keeping its first
        // occurrence, as the later insert keeps what it finds (and as a
        // std::unordered_map made of the range does), and
        ...
        , _equal(equal) {
            ...
            size_t kept = 0;   // the elements of one hash are adjacent, in their order: of two with one key the earlier stays
            for (size_t i = 0; i < order.size();) {
                size_t j = i;
                while (j < order.size() && order[j].hash == order[i].hash) {
                    ++j;
                }
                size_t group = kept;   // this hash's elements kept so far start here (the writes below go no further than the reads)
                for (size_t a = i; a < j; ++a) {
                    bool earlier = false;
                    for (size_t b = group; b < kept && !earlier; ++b) {
                        earlier = _equal(Traits::key(items[order[a].index]), Traits::key(items[order[b].index]));
                    }
                    if (!earlier) {
                        order[kept++] = order[a];
                    }
        ...
        // The element under the key as an iterator, end() when there is
        // none: the walk of find, each level's node and its slots still to
        // visit kept as the iterator keeps them on its way (the slots after
        ...
        const_iterator find_at(const K& key) const {
            ...
            auto hash = _hash(key);
            unsigned depth = 0;
            for (unsigned shift = 0;; shift += Bits, ++depth) {
                auto bit = _bit(hash, shift);
                if (!(node->bitmap & bit)) {
                    return const_iterator();
                }
                auto& entry = hamt_entries<value_type>(node)[_position(node->bitmap, bit)];
                it._nodes[depth] = node;
                if (node->subtries & bit) {
                    it._remaining[depth] = node->bitmap & ~(uint32_t(bit << 1) - 1u);
                    node = entry.link.get();
                    continue;
                }
                it._remaining[depth] = node->bitmap & ~(bit - 1u);
                it._depth = depth;
                if (_equal(key, Traits::key(entry.value))) {
                    it._entry = &entry;
                    return it;
        ...
        // The builder's side (map::builder, set::builder): insert and
        // erase done on this trie in place. A node marked owned
        // (HamtHead::owned) was made by this trie's builder and is reached
        // from nothing but this trie, so a change is made where the node
        // lies, an entry added in a free slot or taken out with the
        // entries after it moved along; a node not marked is shared with
        // the maps the builder was thawed from or froze, and the first
        // change through it copies it, with room for one entry more, and
        // marks the copy, its parent having been made this trie's own
        // before it. Only this trie's builder calls these, and only on a
        ...
        bool erase_in_place(const K& key) {
            ...
            _erase_in_place(_root, 0, _hash(key), key);
            --_size;
            return true;
        }

        // Every node of this trie a map may hold: the marks cleared, down
    ...
    private:
        ...
        // copied into `at`
        // The source node of a copy made reachable in this cycle, through
        // a tracked_ptr to it, whose construction is the barrier: the
        // marking visits the node, which never changes, and marks every
        ...
        // An element moved and its slot kept whole: the builder moves the
        // entries of a node it owns along when it adds or takes one out.
        // Without a move that cannot throw it copies the node instead,
        // as a map's insert does.
        static constexpr bool Relocatable = std::is_nothrow_move_constructible_v<value_type>;

        // The entry `from` moved into the free storage `at`, `from` left
        ...
        static void _relocate(Entry* at, Entry& from, bool element) noexcept {
            if (element) {
                ...
                ::new (static_cast<void*>(at)) Entry(from.link);
            }
            from.~Entry();
        }
        ...
        void _insert_in_place(Link& link, unsigned shift, size_t hash, const K& key, bool& added, A&&... a) {
            ...
            if (!(node.bitmap & bit)) {
                added = true;
                _add_in_place(link, bit, std::forward<A>(a)...);
                return;
            }
            auto& entry = hamt_entries<value_type>(&node)[_position(node.bitmap, bit)];
            if (node.subtries & bit) {
                _insert_in_place(entry.link, shift + Bits, hash, key, added, std::forward<A>(a)...);
                return;
            }
            if (_equal(key, Traits::key(entry.value))) {
                added = false;
                if constexpr (Relocatable) {
                    value_type made(std::forward<A>(a)...);   // what may throw, before the element is touched
                    entry.value.~value_type();
                    ::new (static_cast<void*>(&entry.value)) value_type(std::move(made));
                } else {
                    _put_owned(link, _copy_replacing(node, bit, entry.link, std::forward<A>(a)...));
                }
                return;
            }
            auto ehash = _hash(Traits::key(entry.value));
            if (ehash != hash) {   // apart from here on: a subtrie in place of the element, which takes its chain
                added = true;
                Node sub = _split(shift + Bits, entry, ehash, hash, std::forward<A>(a)...);
                sub->owned = 1;
                entry.value.~value_type();
                entry.link = std::move(sub);
                node.subtries |= bit;
                return;
            }
            // the same hash to the last bit: the chain of the entry, a chain being small and rare, copied as a map's is
            if (Link chain = _chain_replacing(_chain(entry.link.get()), key, std::forward<A>(a)...)) {
                added = false;
                entry.link = chain;
                return;
            ...
            added = true;
            entry.link = make_tracked<Chain>(static_pointer_cast<Chain>(entry.link), std::forward<A>(a)...);
        }

        // An element built from a... in the free slot `bit` of the owned
        // node at `link`: in place when the node has room, the entries
        // after the slot moved up one, last first; a node one entry
        ...
        void _add_in_place(Link& link, uint32_t bit, A&&... a) {
            ...
            if constexpr (Relocatable) {
                if (count < node.capacity) {
                    value_type made(std::forward<A>(a)...);   // what may throw, before the node is touched
                    auto entries = hamt_entries<value_type>(&node);
                    for (auto above = node.bitmap & ~(bit | (bit - 1)); above;) {
                        auto top = uint32_t(1) << (31 - std::countl_zero(above));
                        auto i = _position(node.bitmap, top);
                        _relocate(entries + i + 1, entries[i], !(node.subtries & top));
                        above &= ~top;
                    }
                    ::new (static_cast<void*>(entries + _position(node.bitmap, bit))) Entry(Link(), std::in_place, std::move(made));
                    node.bitmap |= bit;
                    ++node.built;
                    return;
                }
            }
            _put_owned(link, _copy_adding(node, bit, std::forward<A>(a)...));
        }
        ...
        void _remove_in_place(Link& link, uint32_t bit) {
            ...
            if constexpr (Relocatable) {
                auto entries = hamt_entries<value_type>(&node);
                auto i = _position(node.bitmap, bit);
                if (!(node.subtries & bit)) {
                    entries[i].value.~value_type();
                }
                entries[i].~Entry();
                for (auto above = node.bitmap & ~(bit | (bit - 1)); above; above &= above - 1) {
                    auto low = above & (0u - above);
                    auto j = _position(node.bitmap, low);
                    _relocate(entries + j - 1, entries[j], !(node.subtries & low));
                }
                node.bitmap &= ~bit;
                node.subtries &= ~bit;
                --node.built;
            } else {
                _put_owned(link, _copy_removing(node, bit));
            }
        }
        ...
        void _erase_in_place(Link& link, unsigned shift, size_t hash, const K& key) {
            ...
            if (_equal(key, Traits::key(entry.value))) {
                if (auto chain = _chain(entry.link.get())) {   // the chain's first element takes the entry
                    if constexpr (Relocatable) {
                        Link keep = entry.link;
                        value_type made(chain->value);
                        entry.value.~value_type();
                        ::new (static_cast<void*>(&entry.value)) value_type(std::move(made));
                        entry.link = chain->next;
                    } else {
        ...
        static void _disown(HamtHead& node) noexcept {
            ...
            node.owned = 0;
            auto entries = hamt_entries<value_type>(&node);
            for (auto bits = node.subtries; bits; bits &= bits - 1) {
                _disown(*entries[_position(node.bitmap, bits & (0u - bits))].link);
            }
        }
