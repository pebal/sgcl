...
namespace sgcl::immutable {
    namespace detail {
        ...
        // A tail that is not full: a leaf of a type of its own, so of a
        // pool of its own. push_back copies the tail at every push, and
        // pop_back and set do when they change it, and nearly every such
        // copy is dropped at the next push; the leaves of the trie live.
        // In one pool the two shared pages: a vector of a million elements
        // built by push_back held 73 MB of pages after a collection for
        // 8 MB of elements, every page keeping a few leaves of the trie
        // among the slots of dead tails, and a random read walked 32 times
        // the pages it needs. The tail that a push fills is made a
        // VectorLeaf (vector::_push_back), since it is the trie's next leaf
        // and is hung there as it is; one made full otherwise (a set of a
        // full tail) stays a VectorTail in the trie, which costs its page
        // nothing but a place among the tails.
        template<class T>
        struct VectorTail : VectorLeaf<T> {};
    }
    ...
    , public mixin::random_access<vector<T>> {
        ...
        using Tail = detail::VectorTail<T>;
    ...
    public:
        ...
        vector pop_back() const {
            ...
            auto count = _tail_count();
            if (count > 1) {
                return vector(_size - 1, _shift, _root, _copy_leaf<Tail>(*_tail, count - 1));
            }
    ...
    private:
        ...
        vector _push_back(A&&... a) const {
            ...
            if (count == Width - 1) {   // the tail this push fills is the trie's next leaf: made in the trie's pool, hung there as it is
                unique_ptr<Leaf> tail = _copy_leaf(*_tail, count);
                _append(*tail, std::forward<A>(a)...);
                v._tail = std::move(tail);
            } else {
                unique_ptr<Tail> tail = count ? _copy_leaf<Tail>(*_tail, count) : _make_leaf<Tail>();
                _append(*tail, std::forward<A>(a)...);
                v._tail = std::move(tail);
            }
            ++v._size;
        ...
        vector _set(size_type i, U&& value) const {
            ...
            auto offset = _tail_offset();
            if (i >= offset) {
                return vector(_size, _shift, _root, _copy_leaf<Tail>(*_tail, _tail_count(), uint32_t(i - offset), std::forward<U>(value)));
            }
