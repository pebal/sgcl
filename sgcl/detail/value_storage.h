...
namespace sgcl::detail {
    ...
    protected:
        ...
        template<class T>
        static constexpr bool Inline = !MayContainTracked<T>::value && sizeof(T) <= BufferSize && alignof(T) <= BufferAlign && std::is_nothrow_move_constructible_v<T>;
        ...
        struct NodeOps {
            ...
            static T& make(ValueStorage& s, A&&... a) {
                ...
                auto p = ::new(s._word) NodePtr(make_tracked<Node>());
                return (*p)->construct(std::forward<A>(a)...);
            }
            static void copy(ValueStorage& to, const ValueStorage& from) {
                if constexpr(Copyable) {
            ...
            static void destroy(ValueStorage& s) noexcept {
                node(s)->destroy();      // the value now; the node to the collector
                node(s).~NodePtr();
                s._manager = nullptr;
            }
            static void* get(const ValueStorage& s) noexcept {
