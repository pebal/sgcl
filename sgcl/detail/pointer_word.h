...
namespace sgcl::detail {
    ...
    template<class T>
    struct IsWeakPtr<weak_ptr<T>> : std::true_type {};

    // A pointer word: a tracked_ptr or a weak_ptr (a tracked_ptr to its
    // cell). One word that only ever holds null or an
    ...
    template<class T>
    inline constexpr bool IsPointerWord = std::is_base_of_v<Tracked, T> || IsWeakPtr<T>::value;
