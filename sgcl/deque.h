...
namespace sgcl {
    ...
    public:
        ...
        // sweep: its blocks are garbage of the same sweep, possibly
        // destroyed already, and destroy the elements they still hold
        // themselves.
        ~deque() {
            if (_map && !detail::sweeping) {
                _destroy_all();
            }
        }
        ...
        iterator insert(const_iterator pos, size_type count, const T& value) {
            ...
            T copy(value);   // value may live in this deque
    ...
    private:
        ...
        // run: the blocks go with the map as they are.
        void _destroy_all() noexcept {
            if constexpr (!std::is_trivially_destructible_v<T>) {
                auto map = _map.get();
                for (auto index = _start, last = _start + _size; index < last;) {
                    auto block = map[index / BlockSize].get();
        ...
        void _reallocate_map(size_t count, size_t first) {
            ...
            bool with_front = first >= 1 && block >= 1 && _map.get()[block - 1];
            bool with_back = first + used < count && block + used < _map_size && _map.get()[block + used];
            MapPtr map = unique_ptr<BlockPtr>(detail::Maker<BlockPtr[]>::make_tracked_data(count + 1));
            auto src = _map.get() + block;
            auto dst = map.get() + first;
