...
namespace sgcl {
    ...
    namespace detail {
        ...
        // The index of the alternative T; variant_npos when T is none of
        // them or more than one (an access by type is ill-formed then)
    ...
    class variant {
        ...
        template<class T>
        static constexpr size_t index_of = detail::variant_index_of<T, Ts...>;
    ...
    // Ill-formed for a type that is not exactly one alternative, as with
    ...
    namespace detail {
        ...
        // visit over one variant: the visitor invoked with the alternative,
        ...
        R variant_visit(F&& f, V&& v) {
            ...
            return variant_dispatch(v.index(), [&]<size_t I>(std::integral_constant<size_t, I>) -> R {
                return std::invoke(std::forward<F>(f), get<I>(std::forward<V>(v)));
        ...
        // The visitor's result on the first alternative of every variant
        ...
        // Whether the visitor's result is R, the same type, for every
        // combination of the alternatives of Vs: the combinations walked
        // one variant at a time, the alternatives picked so far in the list
        template<class R, class F, class Picked, class... Vs>
        struct VisitUniform;

        // (a requires-expression: a visitor that cannot be invoked on one
        // of the combinations makes it false, not an error)
        template<class R, class F, class... A>
        struct VisitUniform<R, F, TypeList<A...>> : std::bool_constant<requires { requires std::is_same_v<std::invoke_result_t<F, A...>, R>; }> {};
    ...
    // The visitor returns one type for every combination of alternatives,
    // as std::visit requires; a constraint rather than a static_assert,
    // so that a requires-expression sees it. visit<R> converts each
    // result to R instead.
    template<class F, class... Vs>
