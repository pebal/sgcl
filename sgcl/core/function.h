...
namespace sgcl {
    ...
    namespace detail {
        ...
        // A move_only_function of any signature: the class below or one
        // derived from it (deduction takes the base)
        template<class R, bool Const, bool Noexcept, class... Args>
        std::true_type is_move_only_function(const MoveOnlyFunction<R, Const, Noexcept, Args...>*);
        std::false_type is_move_only_function(...);

        template<class F>
        inline constexpr bool IsMoveOnlyFunction = decltype(is_move_only_function(std::declval<F*>()))::value;

        // A null function pointer, member pointer or empty function: what
        // std::function treats as nothing to hold. A function of either
        // library is known by its target_type, a move_only_function (which
        // has none) by its class.
        template<class F>
        bool callable_is_null(const F& f) noexcept {
            if constexpr(std::is_pointer_v<F> || std::is_member_pointer_v<F>) {
                return f == nullptr;
            } else if constexpr(IsMoveOnlyFunction<F> || requires { typename F::result_type; f.target_type(); static_cast<bool>(!f); }) {
                return !f;
            } else {
                return false;
            }
