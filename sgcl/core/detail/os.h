...
// store with release on the mutator, a plain or relaxed load and an
// acquire fence on the collector: the states of a page before it is let
// go of), which the thread sanitizer does not model. Where such a pair
// is known, the two sides tell it so with an annotation on the page, a
// happens-before edge in its eyes and nothing in any other build.
#if defined(__has_feature)
#if __has_feature(thread_sanitizer)
#define SGCL_TSAN 1
#endif
#endif
#if !defined(SGCL_TSAN) && defined(__SANITIZE_THREAD__)
#define SGCL_TSAN 1
#endif
#if defined(SGCL_TSAN)
#include <sanitizer/tsan_interface.h>
#define SGCL_TSAN_RELEASE(p) __tsan_release((void*)(p))
#define SGCL_TSAN_ACQUIRE(p) __tsan_acquire((void*)(p))
#else
#define SGCL_TSAN_RELEASE(p) ((void)0)
