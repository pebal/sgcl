...
namespace sgcl::detail {
    struct alignas(config::CacheLineSize) Page {
        ...
        // (detail/pointer.h: the Released constructor, store_released), in
        // two states around the word and the card. First Releasing, with
        // the parity the allocator read (unique_state): a root in either
        // parity, as UniqueLock is, from the registration on (collector.h:
        // _register_page queues the roots by state it finds) and in every
        // states pass. Then, once the word and the card are stored, the
        // released state: Reachable with the current parity and Fresh, the
        // state of an object made after the flip that the cycle leaves
        ...
        // thread's stack have been scanned already. A release store: the
        // collector that reads this state has the word and the card.
        //   Why two states: a thread preempted between its read of the
        // epoch and the store, across the flip, stored the old parity,
        // which no pass takes for a root, and with the word not stored
        // yet the object was reachable through nothing the cycle read
        // (measured on the stepper's gates). In Releasing until the word
        // is in, the object is a root of the registration or of the pass
        // whichever the stale store comes after, and marked objects are
        // not swept; a stale state that lands before the registration has
        // the word and the card in front of it. No fence and no word of
        // the thread: the byte the release stores anyway, stored twice.
        // The slot found once for the two states (page_of and index_of, a
        // table load and a multiply: found twice, the release cost the
        // allocation benchmark 7%)
        struct Release {
            Page* page;
            std::atomic<State>* state;
            State parity;   // the allocation's
        };

        static Release set_state_releasing(const void* p) noexcept {
            assert(p != nullptr);
            auto page = Page::page_of(p);
            auto& state = page->states()[page->index_of(p)];
            auto old = state.load(std::memory_order_relaxed);
            assert(is_unique_state(old));
            auto parity = State(old & State::Parity);
            state.store(State(State::Releasing | parity), std::memory_order_relaxed);
        ...
        static void set_state_released(const Release& r) noexcept {
            ...
            if (!r.page->state_updated.load(std::memory_order_relaxed)) {
                r.page->state_updated.store(true, std::memory_order_release);
            }
        }
        ...
        uint8_t retire = {0};   // collector: the sweeps after which the states of the other parity are retired, 2 where state_updated is lowered (the registration) and counted down by each retire (_retire_states)
