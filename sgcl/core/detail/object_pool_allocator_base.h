...
namespace sgcl::detail {
    ...
    public:
        ...
        // (page.h: unique_state), and with the page's object_created flag
        // raised: the maker need not look the page up. The flag is stored
        // on every allocation, after the state, not only when it reads as
        // down: the registration lowers it with an exchange (acq_rel) and
        // reads the states right after, so a store of the flag skipped on
        // a read that saw it still up could leave the collector without
        // the state (still in this thread's store buffer) and without the
        // flag, the slot unregistered until another allocation on the
        // page and its targets, held by it alone, lost at the next sweep.
        // Stored unconditionally, the exchange either reads this store
        // (synchronizing with it: the state is visible to the reads that
        // follow) or precedes it, leaving the flag up for the next
        // registration. The line is this thread's while it allocates: a
        // byte store, no atomic operation, and the load with its branch
        // gone from the loop (measured, benchmarks/core/allocation.cpp: 5.0 to
        // 3.4 ns per allocation on one thread, 9.3 to 7.2 on 24).
        ...
        // The store of the flag is the release that publishes the init and
        // the state, of this allocation and of every one before it on
        // this thread: the registration's exchange (and the acquire load
        // of _register_late) synchronizes with it before the states are
        // read, so the state itself is stored relaxed, as the state of a
        // large object is (page.h: set_state<UniqueLock>). What the
