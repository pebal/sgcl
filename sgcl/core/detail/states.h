    ...
    struct States8 {
        ...
        static unsigned reachable(uint64_t w, State current) noexcept {
            return bits((match(w & ~spread(State::Fresh), current)
                       ...
                       | match(w & ~spread(State::Parity), State::Releasing)
        ...
        // Releasing of either parity: leaving its unique_ptr, a root as
        // UniqueLock is (types.h)
        static unsigned releasing(uint64_t w) noexcept {
            return bits(match(w & ~spread(State::Parity), State::Releasing) >> 7);
        }
