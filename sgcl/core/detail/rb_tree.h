...
namespace sgcl::detail {
    ...
    protected:
        ...
        // A handle dying in a sweep, inside a managed object nobody refers
    ...
    public:
        ...
        // One by one with end() as the hint. An element of the value type
        // is compared as it is and copied into its node once its place is
        // known; a range of another type (string_views for string keys,
        // pairs of a key and a value for a map) is converted once per
        // element, into the node first, and the node's key compared, as
        // emplace_hint does: the source may not be comparable with the
        // keys at all, and where it is, a conversion per comparison would
        // build a temporary key each time.
        template<std::input_iterator InputIt>
        void insert(InputIt first, InputIt last) {
            ...
            for (; first != last; ++first) {
                if constexpr (std::is_same_v<std::remove_cvref_t<std::iter_reference_t<InputIt>>, value_type>) {
                    _insert_hint(_hdr(), *first);
                } else {
                    _emplace_hint(_hdr(), *first);
                }
            }
        }
    ...
    private:
        ...
        // order and the size; for the tests (RbTreeCheck)
        friend struct RbTreeCheck;
    ...
    protected:
        ...
        // The elements of `other`, shape for shape: a copy of `other`'s
        // tree, node for node with its colour and its three links, with
        // no comparison and no rebalancing (the copy has the same header
        // extremes and the same root). A throwing element copy leaves
        // the nodes made so far linked from the header, and the tree is
        // cleared (the copy constructor and copy assignment).
        void _copy_from(const RbTree& other) {
            if (other.empty()) {
                return;
            }
            try {
                _ensure_header();
                NodeBase* h = _hdr();
                _copy_subtree(other._root(), h, h->parent);
                h->left.reset(rb_minimum(h->parent.get()));
                h->right.reset(rb_maximum(h->parent.get()));
                _size = other._size;
            }
            catch (...) {
                clear();
        ...
        // The subtree at x copied under p, hung from `link` (p's left or
        // right, or the header's parent for the root). Recursion on the
        // right subtree only, the left a loop, as _destroy_subtree walks.
        // A node is linked from its parent before the next is made, so
        // that the new tree roots it from its header; its own children
        // are null from the page.
        void _copy_subtree(const NodeBase* x, NodeBase* p, Ptr& link) {
            Ptr* at = &link;
            while (x) {
                auto n = make_tracked<Node>();   // rooted by its state (UniqueLock) and by this frame until it is linked
                Node* node = n.get();
                node->slot.construct(static_cast<const Node*>(x)->slot.value);
                node->red = x->red;
                node->parent.reset(p);
                *at = std::move(n);   // released from its unique_ptr into the tree
                _copy_subtree(x->right.get(), node, node->right);
                x = x->left.get();
                p = node;
                at = &node->left;
            }
        ...
        // is rooted until its turn. The left child is anchored before its
        // parent is unlinked: the element's destructor runs between the
        // two, user code that may allocate and take long, and a raw
        // pointer may sit in a register the conservative scan does not
        // see.
        static void _destroy_subtree(NodeBase* first) noexcept {
            ...
            while (x) {
                ...
                auto y = x->left.get();
                next.reset(y);   // rooted by x until here
                _unlink(x);
                _node(x)->slot.destroy();
                keep.reset(y);   // the dead x let go
                x = y;
            }
        }
        ...
        iterator _insert_at(const InsertPos& pos, A&&... a) {
            ...
            // the node are stored (reset() refuses a unique object), the
            // released state once they are, as a tracked_ptr's assignment
            // from a unique_ptr does it (page.h: set_state_releasing)
            auto release = Page::set_state_releasing(n.get());
            _link(pos, n.get());
            Page::set_state_released(release);
