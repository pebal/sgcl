...
namespace sgcl::detail {
    ...
    // config::backoff_max. Many threads at one word otherwise spend more
    ...
    // config::backoff_max for a word that is the whole structure (the
    // stack's head), less where a long pause leaves work undone that
    // others wait for (the channel's ring).
    template<unsigned Max = config::backoff_max>
    struct Backoff {
        unsigned pauses = 1;
