...
namespace sgcl {
    ...
    private:
        Link _head;   // the sentinel, a bare NodeBase without an element
        ...
        // Destroys the element of the node after `prev` and unlinks it. The
        // node is held by a tracked pointer on this stack meanwhile: the
        // caller's raw pointer may live in a register only, which the
        // collector does not see, and once unlinked the node has no other
        // reference while its link is cleared (a stale word pointing at the
        // node then retains nothing else).
        ...
        // empty range, (prev, prev) included, is a no-op. Each node is held
        // by a tracked pointer on this stack while it is cleared, and the
        // link to `last` is copied from that holder once the walk gets there.
