...
namespace sgcl {
    ...
    private:
        ...
        // it is linked in, its destructor destroys the elements and drops
        // the links, as _unlink does: a throwing element constructor leaves
        // the list as it was, and a stale word pointing at one node of the
        // run keeps no other.
        struct Chain {
            Link first;
            Link last;
        ...
        // Unlinks one linked node and destroys its element. The node is held
        // by a root of this frame meanwhile: the caller's raw pointer may
        // live in a register only, which the collector does not see, and
        // once unlinked the node has no other reference. Destroyed is the
        // last store to the node: the root is the frame's word and the state
        ...
        // [first, last) unlinked and their elements destroyed. A null
        ...
        NodeBase* _erase(NodeBase* first, NodeBase* last) {
            ...
            _size -= _release(first, last);
            return last;
        }

        // Destroys the elements of the detached nodes first..(last) and
        // drops their links (see _unlink); returns how many. Two roots of
        // this frame alternate along the walk: a node is rooted from before
        // its predecessor lets go of it, through its element's destructor
        // (user code, with the node reachable from nowhere else), until
        // its slot's Destroyed, the last store to it (_erase_one). One root
        // would not do: moved on to the next node before this one's
        // destructor runs, it leaves the node to a raw local, possibly a
        // register, which is no root, and a sweep finding the node unmarked
        // and not yet Destroyed would run its destructor a second time. The
        // cost is one set_state<Reachable> per node.
        static size_type _release(NodeBase* first, NodeBase* last) noexcept {
