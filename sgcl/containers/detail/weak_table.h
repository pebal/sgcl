...
namespace sgcl::detail {
    ...
    protected:
        ...
        // The table with its keyed emplace in reach: the object looked up
        // by its pointer (the hash and the equality are transparent to it)
        // and the entry made from it only when there is none, one search
        // where a find and an emplace were two
        struct Opened : Table {
            using Table::Table;
            using Table::_try_emplace;
            using Table::_make_iterator;
        };
