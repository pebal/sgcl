...
namespace sgcl {
    ...
    public:
        using key_type = Key;
        using mapped_type = V;
        using value_type = pair<const Key, V>;
        using key_compare = Compare;
        using size_type = size_t;
        using difference_type = ptrdiff_t;

    private:
        // What a node stores (vector.h, detail/managed.h): the value with
        // a gc::tracked_ptr mapped to the word it names; the interface,
        // the iterators included, sees value_type over it
        using Value = detail::managed_value_t<value_type>;
        ...
        public:
            ...
            operator Iterator<const concurrent_map::value_type>() const noexcept {
                return Iterator<const concurrent_map::value_type>(_node);
            }
        ...
        private:
            ...
            template<class, class, class, template<class> class> friend class concurrent_map;
        };
    ...
    public:
        ...
        concurrent_map(InputIt first, InputIt last, const Compare& comp = Compare())
        : concurrent_map(comp) {
            insert(first, last);
        }

        concurrent_map(std::initializer_list<value_type> ilist, const Compare& comp = Compare())
        : concurrent_map(ilist.begin(), ilist.end(), comp) {
        }
        ...
        // Lookup: wait-free, the element as it was found
        iterator find(const Key& key) noexcept {
            tracked_ptr<NodeBase> node = _search(key);
            return iterator(node && !_comp(key, _key(node.get())) ? node : tracked_ptr<NodeBase>());
        }

        const_iterator find(const Key& key) const noexcept {
            tracked_ptr<NodeBase> node = _search(key);
            return const_iterator(node && !_comp(key, _key(node.get())) ? node : tracked_ptr<NodeBase>());
        }
        ...
        // node when the key turns out to be taken; try_emplace looks the
        // key up first and builds nothing when it is there.
        template<class... A>
        ...
        pair<iterator, bool> try_emplace(const Key& key, A&&... a) {
            ...
            unsigned h = _height();
            tracked_ptr<NodeBase> node = _make_node(h, std::piecewise_construct, forward_as_tuple(key), forward_as_tuple(std::forward<A>(a)...));
            return _insert(node, h);
        }
        ...
        pair<iterator, bool> try_emplace(Key&& key, A&&... a) {
            ...
            unsigned h = _height();
            tracked_ptr<NodeBase> node = _make_node(h, std::piecewise_construct, forward_as_tuple(std::move(key)), forward_as_tuple(std::forward<A>(a)...));
            return _insert(node, h);
        }
        ...
        size_type erase(const Key& key) {
            ...
            return _erase(tracked_ptr<NodeBase>(succs[0])) ? 1 : 0;
        }
    ...
    private:
        ...
        unsigned _height() noexcept {
            ...
            unsigned top = _top.load(std::memory_order_relaxed);
            if (h > top + 1) {
                h = top + 1;
            }
            while (h > top && !_top.compare_exchange_weak(top, h, std::memory_order_relaxed)) {
            }
            return h;
        }
        ...
        bool _find(const Key& key, NodeBase** preds, NodeBase** succs, unsigned& top) {
        retry:
