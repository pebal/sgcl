...
namespace sgcl {
    ...
    // The atomic view of a gc::tracked_ptr: the operations of atomic_ref<tracked_ptr<T>>
    // on the word the gc::tracked_ptr resolves to (ptr.h: _word), found once in
    // the constructor. As with atomic_ref<tracked_ptr<T>>, the gc::tracked_ptr must not be
    // moved or destroyed while a view of it exists.
    template<class T>
