...
gc::any a = gc::vector<gc::tracked_ptr<Node>>{gc::make_tracked<Node>(2)};   // the vector in a managed object of its own
if (auto p = gc::any_cast<gc::vector<gc::tracked_ptr<Node>>>(&a)) {
    std::cout << (*p)[0]->value << "\n";
}
gc::tracked_ptr node = gc::make_tracked<Node>(3);
gc::function<int()> f = [node] { return node->value; };   // the closure in a managed object of its own: node lives while f holds it
