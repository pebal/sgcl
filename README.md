...
- **Lock-free containers**: `concurrent_queue` (Michael–Scott), `concurrent_stack` (Treiber), `concurrent_map` and `concurrent_set` (a skip list, the structure of Java's `ConcurrentSkipListMap`), `concurrent_unordered_map` and `concurrent_unordered_set` (a split-ordered list, the resizable lock-free hash table) and `copy_on_write<T>` (a value read by many and replaced whole, Java's `CopyOnWriteArrayList` for any type), the textbook algorithms with no reclamation scheme in them, because the collector is one: what `java.util.concurrent` has and C++ has had only with hazard pointers or epochs written by hand.
