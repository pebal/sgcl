...
- **Lock-free containers**: `concurrent_queue` (Michael–Scott), `concurrent_stack` (Treiber), `concurrent_map` and `concurrent_set` (a skip list, the structure of Java's `ConcurrentSkipListMap`), `concurrent_unordered_map` and `concurrent_unordered_set` (a split-ordered list, the resizable lock-free hash table), the textbook algorithms with no reclamation scheme in them, because the collector is one: what `java.util.concurrent` has and C++ has had only with hazard pointers or epochs written by hand.
...
- An `sgcl::` container lives where a `tracked_ptr` may, on a stack or inside a managed object; the `gc::` one anywhere, a global included, paying the test of the mode on its root words. Iterators of the maps and sets are of the same kind as their container.
