...
| [concurrent](docs/sgcl/concurrent/README.md) | sync | `atomic<tracked_ptr>` and `atomic_ref` with compare-exchange and no ABA; `concurrent_queue` (Michael–Scott), `concurrent_stack` (Treiber), `concurrent_sorted_map` and `concurrent_sorted_set` (a skip list), `concurrent_map` and `concurrent_set` (a split-ordered list): the textbook algorithms with no reclamation scheme in them, because the collector is one; `copy_on_write` |
