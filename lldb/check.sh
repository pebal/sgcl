...
check '(gc::tracked_ptr<Node>) node = 0x'
check '(tracked'
check 'cell 0 of block 0x'
check 'v = 8'
check '(sgcl::unique_ptr<Node>) owned = 0x'
check 'UniqueLock'
check '(sgcl::weak_ptr<Node, gc::tracked_ptr>) weak = 0x'
check '(gc::vector<int>) v = size=3 capacity='
check '[2] = 3'
check '[1] = null'
check '(gc::array<int>) a = size=3'
check '[0] = 5'
check '(gc::deque<int>) d = size=3'
check '[0] = 9'
check '[1] = "bb"'
check '(gc::forward_list<int>) f = size=2'
check '(first = 1, second = "one")'
check '(gc::set<int>) s = size=3'
