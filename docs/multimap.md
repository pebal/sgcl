...
```cpp
sgcl::multimap<std::string, int> scores = {{"ann", 3}, {"bob", 5}, {"ann", 7}};     // ann 3, ann 7, bob 5
sgcl::multimap<int, int, std::greater<int>> desc(std::greater<int>{});
sgcl::multimap<std::string, int> taken = std::move(scores);                         // scores is empty now
```
...
```cpp
sgcl::multimap<std::string, int> m = {{"b", 2}, {"a", 1}, {"a", 3}};
...
```cpp
sgcl::multimap<int, std::string> m = {{1, "a"}, {1, "b"}};
...
```cpp
sgcl::multimap<std::string, std::string> m;
m.emplace("k", "v");
...
```cpp
sgcl::multimap<int, std::string> m = {{1, "a"}, {1, "b"}, {2, "c"}};
...
```cpp
sgcl::multimap<std::string, int, std::less<>> m = {{"a", 1}, {"a", 2}};
...
struct Event {
    std::string what;
    sgcl::tracked_ptr<Event> cause;     // traced through the node that holds the Event
};

int main() {
    // A multimap on the stack: several events per day, in the order they came
    sgcl::multimap<int, sgcl::tracked_ptr<Event>> log;
    sgcl::tracked_ptr first = sgcl::make_tracked<Event>("boot");
    log.emplace(1, first);
    log.emplace(1, sgcl::make_tracked<Event>("login", first));
    log.emplace(2, sgcl::make_tracked<Event>("logout"));
    log.emplace(2, sgcl::make_tracked<Event>("shutdown"));
    log.emplace(1, sgcl::make_tracked<Event>("late entry"));     // goes after the other day-1 events
