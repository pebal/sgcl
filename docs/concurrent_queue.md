...
- A `tracked_ptr` may not address an element ([The rules](../README.md#the-rules), 4); there is no `front()`: the first element is what `try_pop` returns.
...
- [queue](queue.md), the sequential adapter
