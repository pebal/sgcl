...
```cpp
#include "sgcl/atomic_ref.h"   // or "sgcl/sgcl.h"

namespace sgcl {
    template<class T>
    class atomic_ref<tracked_ptr<T>>;
    template<class T>
    class atomic_ref<gc::tracked_ptr<T>>;
}
```

`atomic_ref<tracked_ptr<T>>` is `std::atomic_ref` for a [`tracked_ptr`](tracked_ptr.md): the operations of [`atomic<tracked_ptr<T>>`](atomic.md) (`load`, `store`, `compare_exchange_weak`, `compare_exchange_strong`, `wait`, `notify`, with a `std::memory_order`) applied to a plain `tracked_ptr` that lives somewhere already: a member of a node, an element of an `sgcl::vector<tracked_ptr<T>>`, a local. The word of a `tracked_ptr` is a `std::atomic` of a pointer in any case, so nothing changes in the pointer's layout; the `atomic_ref` is a reference to it and the operations are the atomic ones, with the hazard pointer of `atomic::load` and the same freedom from ABA. `atomic_ref<gc::tracked_ptr<T>>` is the same for a [`gc::tracked_ptr`](gc/tracked_ptr.md), with `gc::tracked_ptr<T>` as its `value_type`: the constructor binds the word the `gc::tracked_ptr` holds its object by, the `tracked_ptr` itself inside a managed object or on a stack, the cell's word in any other memory, and the operations are those of `atomic_ref<tracked_ptr<T>>` on that word, with `tracked_ptr<T>` parameters as in [`atomic`](atomic.md) and the `value_type` coming out; the `gc::tracked_ptr` must not be moved or destroyed while a view of it exists, as with any `atomic_ref`. Only these two specializations exist.
...
- As with `std::atomic_ref`, while any `atomic_ref` to a `tracked_ptr` exists, the pointer is accessed only through `atomic_ref`s: a plain copy or assignment of the pointer at the same time is a data race in the program's terms (the word itself is atomic, so it is never torn, and the collector is correct under any interleaving: [The rules](../README.md#the-rules), 6).
...
- In a destructor the referred `tracked_ptr` is a member of a dying object: its target may be dying in the same sweep, so a destructor does not `load()` it ([The rules](../README.md#the-rules), 5).
...
```cpp
struct Node { gc::tracked_ptr<Node> next; };
gc::tracked_ptr node = gc::make_tracked<Node>();
gc::atomic_ref next(node->next);     // atomic_ref<tracked_ptr<Node>>, deduced
gc::atomic_ref same = next;          // the same word
next.store(node);
...
The `tracked_ptr` referred to, a public member: for `atomic_ref<gc::tracked_ptr<T>>` the word the `gc::tracked_ptr` holds its object by.
...
```cpp
gc::tracked_ptr<int> word;
gc::atomic_ref a(word);
a = gc::make_tracked<int>(1);     // a store
gc::tracked_ptr<int> p = a;       // a load
a = nullptr;
assert(*p == 1 && !a.load());
```
...
```cpp
gc::tracked_ptr word = gc::make_tracked<int>(1);
gc::tracked_ptr p = gc::atomic_ref(word).load(std::memory_order_acquire);       // the 1, held
assert(*p == 1);
```
...
```cpp
gc::tracked_ptr<int> word;
gc::atomic_ref(word).store(gc::make_tracked<int>(1));       // from a unique_ptr
gc::tracked_ptr two = gc::make_tracked<int>(2);
gc::atomic_ref(word).store(two, std::memory_order_release);
gc::atomic_ref(word).store(nullptr);
```
...
struct Stack { gc::tracked_ptr<Item> head; };     // a plain member, used atomically through atomic_ref
gc::tracked_ptr stack = gc::make_tracked<Stack>();

gc::atomic_ref head(stack->head);
...
```cpp
struct Slot { gc::tracked_ptr<int> value; };
gc::tracked_ptr slot = gc::make_tracked<Slot>();
std::thread producer([&slot] {                      // a reference: the tracked_ptr stays on this frame
    gc::atomic_ref(slot->value).store(gc::make_tracked<int>(1));
    gc::atomic_ref(slot->value).notify_one();
});
gc::atomic_ref(slot->value).wait(nullptr);          // until the slot is not null
assert(*gc::atomic_ref(slot->value).load() == 1);
producer.join();
```
...
`sgcl::atomic_ref(p)` for a `tracked_ptr<T> p` is an `atomic_ref<tracked_ptr<T>>`.
...
int main() {
    gc::vector<gc::tracked_ptr<Item>> slots(64);       // 64 null slots in a managed buffer

    std::vector<std::thread> workers;
    for (int t = 0; t < 4; ++t) {
        workers.emplace_back([&slots, t] {              // a reference: the vector stays in main's frame
            for (size_t i = 0; i < slots.size(); ++i) {
                gc::tracked_ptr<Item> expected;         // null: the slot is free
                gc::tracked_ptr mine = gc::make_tracked<Item>(t);
                gc::atomic_ref slot(slots[i]);
...
- [tracked_ptr](tracked_ptr.md), [unique_ptr](unique_ptr.md), [make_tracked](make_tracked.md), [vector](vector.md)
- README: [SGCL classes](../README.md#sgcl-classes), [The rules](../README.md#the-rules), [Threads](../README.md#threads)
