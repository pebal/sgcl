...
- The cost of a push at either end: a load of the map, a load of the block, the construction, two stores (the block's range and the count); the map at its end or a missing block goes the slow way ([Benchmarks: Containers](../README.md#containers-1): 2.0 ns per push against 1.5 ns for `std::deque`).
