...
A value for the object, `T(a...)` for `emplace`, unless the object has one; whether one was added. `insert_or_assign` replaces the value the object has. Every insertion counts towards the next sweep.
...
The entry of the object, dropped: how many (0 or 1). By iterator: the next live entry.
...
- `tests/weak_map.cpp`: every behaviour above, checked.
