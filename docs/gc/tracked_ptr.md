...
- A `gc::tracked_ptr` in unmanaged memory owns its cell for its whole life: a `gc::tracked_ptr` that outlives the thread or the program phase that made it keeps its object exactly as long as itself, no longer, and a pointer moved from keeps its cell and its value, as a moved-from `sgcl::tracked_ptr` does.
...
As the constructors of `sgcl::tracked_ptr`, plus one from an `sgcl::tracked_ptr<U>`. Every constructor first decides the mode from its own address (the heap's range, then the thread's stack, which registers the thread on first contact). In unmanaged memory every constructor makes the pointer's cell, a null included. A move is a copy, as for `sgcl::tracked_ptr`: the source keeps its value and its cell.
...
A `tracked_ptr`'s destructor where the word is one; in unmanaged memory the cell, if any, is destroyed at once, and the object lives on only if something else reaches it.
