...
What it costs, against `sgcl::tracked_ptr` (one thread, `benchmarks/gc_tracked_ptr.cpp`): a copy onto the stack 1.8 ns against 1.2 (a thread-local read for the stack bounds), a store into a member of a managed object 1.6 against 1.5 (the test of the mode), a dereference 0.47 against 0.43 (the test of the sign). In unmanaged memory every construction takes a cell, a null included, and every store afterwards is a store with the barrier into it, 2.2 ns; a construction and destruction together, a cell taken and given back, 7.3 ns (13.4 when every cell was a managed object of its own). A `std::vector<gc::tracked_ptr<T>>` of a million elements is a million cells in 62,500 blocks of one line, 8 bytes per pointer; its reallocation takes a million and gives back a million, and the blocks given back go with the next cycle. A long-lived pointer pins its block, one line, for as long as it lives.
