...
- The cost of `push_back`: a compare of two words of the vector object, a store of the element and a store of the count; the growth is a cold call. The old buffer of a reallocation is collected, not freed at once, so the capacity doubles to halve what waits ([Benchmarks: Containers](../README.md#containers-1): 3.1 ns per `push_back` against 2.0 ns for `std::vector`).
