# sgcl::async::sleep, sgcl::async::sleep_until, sgcl::async::after, sgcl::async::at, sgcl::async::tick, sgcl::async::timeout

```cpp
#include "sgcl/async/timer.h"   // or "sgcl/sgcl.h"

namespace sgcl {
    class duration;                                           // core/duration.h: nanoseconds, Go's text; 1s, 500ms and every integral std::chrono duration convert into it
...
```cpp
explicit sleep(duration d) noexcept;          // an awaitable: co_await async::sleep(d)
...
```cpp
template<class F> auto async::timeout(duration d, F f);     // f(): the time came before any other case
template<class F> auto async::timeout(time_point t, F f);   // the same at a point
```
...
async::task<int> poll(async::channel<int>& data, async::channel<void>& stop) {
    ...
    co_await async::select(
        ...
        async::timeout(20ms, [&] { ++late; })
    );
    co_return seen + late * 1000;
}

int main() {
    async::channel<int> data(8);
    async::channel<void> stop;
    auto p = async::spawn(poll(data, stop));
    for (int i : range(5)) {
        data.send(i).wait();
