...
`get()` works from a plain function because the thread knows which frame it runs: the scheduler, an executor and `async::task::resume()` set a thread-local to the frame around every resume (`detail::current_frame`). The same thread-local is what a start reads to inherit from, so a task started from a plain thread, outside any task, inherits nothing.

## Rules

- A `async::task_local<T>` is a static, one per key; it is not copyable. Its values are in managed nodes: `T` may hold a `tracked_ptr` and lives as a member of a managed object does.
...
template<class U> async::task<U> with(T value, async::task<U> t);   // a task that runs t with the value set
```
...
```cpp
async::task_local<tracked_ptr<User>> current_user;

async::task<> handle(tracked_ptr<Request> r) {
    ...
    async::go(audit(r));                           // a detached task: inherits it too
    co_await current_user.with(admin, repair(r)); // repair runs as admin; this task stays r->user
}

async::task<> validate(tracked_ptr<Request> r) {
...
- [coroutine](coroutine.md): `task`, the frame's header; [managed_frame](../core/coroutine.md): the managed frame under it; [scheduler](scheduler.md): `spawn`, `go`
