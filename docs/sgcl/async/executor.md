# sgcl::executor, sgcl::strand, sgcl::on, sgcl::on_workers
...
namespace sgcl {
    ...
    template<class T, class Executor> task<T> spawn(task<T> t, Executor& ex);   // ex.spawn(t)
    template<class T, class Executor> void go(task<T> t, Executor& ex);         // ex.go(t)
}
```

The same in the `Sgcl` interface: [Executor, Strand, On, OnWorkers](../Sgcl/Async/Executor.md).

The [scheduler](scheduler.md) runs a task on whichever worker is free. A platform's UI toolkit (Cocoa, Win32, GTK, the browser) and many a C library demand one thread, usually the main one, for every call into them; an `executor` is a queue of frames that only the thread running the executor resumes. `ex.run()` is that thread's loop: what is queued, run to its next suspension; nothing queued, parked; until `stop()`. `ex.run(task)` is the loop until the task is done, and the task's result, which makes a program one task on the main thread: `int main() { sgcl::executor main; return main.run(program()); }`. `ex.poll()` is one pass over what is queued and a return, for a loop that is not the library's, a `CFRunLoop`, a `GMainLoop`, a game's frame, which calls it when it has a moment. A task moves to an executor with `co_await sgcl::on(ex)` (Kotlin's `withContext(Dispatchers.Main)`) and back to the pool with `co_await sgcl::on_workers()` (`withContext(Dispatchers.Default)`); `ex.spawn(t)` starts a task on it.

A task on an executor stays on it. Its frame remembers the executor (two words at the front of every managed frame's buffer, `detail::FrameHeader`: the executor and the task-locals), and every wake of the frame, whatever woke it, a channel, a timer, a task it awaited, a `yield`, a `select`, an event, goes through the scheduler's `enqueue`, which routes the frame to the executor's queue when it has one. So a task that does `co_await sleep(1s)` on the main thread is resumed on the main thread, though the timer fired on the timer thread; a task on the main thread that awaits a channel is resumed on the main thread, though a worker served it. A task that the task awaits (`co_await f()` of a task nobody started) runs where the awaiter runs, as a call would: on the main thread for a main-thread task, on the strand for a strand's task, so that what the awaiter guarantees holds for what it awaits; a task it starts with `sgcl::spawn` or `sgcl::go` runs on the pool, one it starts with `ex.spawn` or `ex.go` on the executor.
...
The executor's thread parks when the queue is empty, on the queue's own word, and a push wakes it, after the spin of `config::WorkerSpinMicroseconds` every worker does before it sleeps, so that a task that hops to the main thread and back does not pay two trips through the kernel. What a hop costs, measured once on an M-series laptop: a round trip `co_await on(ex)` from a worker to an executor and `co_await on_workers()` back, about 1 µs; a hop between two strands about 240 ns; a `yield` on an executor or a strand about 200 ns (the queue is a [concurrent_queue](../concurrent/concurrent_queue.md), a managed node per push), against 23 ns for a `yield` on a worker (a ring).
...
- A task on a worker waits with `co_await`, and so does a task on an executor: a blocking call there (`ch.receive()`, `t.join()`) blocks the executor's thread and every task queued on it.
...
void run(task<> t);
template<class T> void run_until(task<T>& t);  // the loop until t is done (or stop()); starts t here if nobody has
...
`co_await sgcl::on(ex)`: the task goes on on the executor from the next line, and what it awaits from then on wakes it there; at once, without a hop, when it is there already. From a strand to another strand, from an executor to a strand, from the pool to either.
...
- [coroutine](coroutine.md): `task`, `managed_frame`, the frame's header; [mutex](mutex.md): what must hold across a wait
