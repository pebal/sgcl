# sgcl::clock, sgcl::manual_clock

```cpp
#include "sgcl/async/timer.h"   // or "sgcl/sgcl.h"

namespace sgcl {
    struct clock {                                             // the module's clock: a Clock of the standard library
...
- One manual clock installed at a time (asserted); the destructor uninstalls, so `sgcl::manual_clock c; c.install();` at the top of a test covers the test. Written unqualified under `using namespace sgcl`, the name `clock` collides with `::clock` of `<ctime>`: write `sgcl::clock::now()`.
...
- The manual clock serves the timers of this module: a `sleep`, `sleep_until`, `after`, `at`, `tick`, `timeout`, `stop_after`, and a thread's `sleep(d).wait()` and `sleep_until(t).wait()`. `sgcl::this_thread::sleep_for` is the operating system's and sleeps for real; the reactor's waits are the kernel's.
...
The module's time: the steady clock's, or the manual clock's while one is installed.
...
- [timer](timer.md): what the clock serves: `sleep`, `sleep_until`, `after`, `at`, `tick`, `timeout`; [stop_token](stop_token.md): `stop_after`, a deadline through the clock
