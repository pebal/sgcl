...
namespace sgcl::io {
    ...
    result<string> look_path(const string& file);   // the executable a name stands for (exec.LookPath)
}
```
...
cmd.dir = "/repo";
auto out = cmd.output();                       // result<string>: the standard output, or the error
if (!out) {
    std::cerr << out.error().message() << ' ' << cmd.captured_err;   // "wait /usr/bin/git: the process ended with a failure status"
}
```

The child is made with `posix_spawn`, never `fork`: a fork of a process with the collector's threads would hold their locks in a child that has none of them. Its exit is waited for on the [reactor](../async/reactor.md) (`exited(pid)`: kqueue's `EVFILT_PROC`, a pidfd on Linux), so `async_wait()` holds no thread while the child runs, as Go 1.23 waits on Linux; `wait()` from a thread is `waitpid`. A `command("true").run()` costs 1.1 ms on an Apple M-series core against Go's 1.9 (Go forks on macOS), 32 at once 0.4 ms each against 0.8 (`bench_io`, `benchmarks/go/exec`).
...
- `start()` once; `wait()` once, from a thread (`waitpid`) or a task (`async_wait()`, on the reactor). A child ended with a failure status is `errc::exit_status` from `wait` (Go's `ExitError`), the code in `state`; a child never waited for stays a zombie until the program ends, as everywhere.
- `in`, `out`, `err`: null is the null device (Go's nil), as `run()` leaves them; a `file` is the descriptor itself (`os::stdout()` to share the program's); a `buffer` or any other stream goes through a pipe and a task, started by `start()` and joined by `wait()`. The same writer in `out` and `err` gets one pipe, so the child's two streams interleave as written.
...
tracked_ptr<reader> in;                         // standard input: null the null device, a file its descriptor, else a pipe and a task
tracked_ptr<writer> out;                        // standard output: the same
tracked_ptr<writer> err;                        // standard error: the same; the same object as out shares its pipe
bool set_pgid = false;                          // a process group of its own (Setpgid)
stop_token stop;                                // the child killed on the stop (CommandContext)
...
result<void> start();                           // look_path, the streams arranged, posix_spawn; once
result<void> wait();                            // waitpid on this thread, the tasks joined, the pipes closed; errc::exit_status for a failure status
task<result<void>> async_wait();                // the same from a task: the exit on the reactor
result<void> run();  task<result<void>> async_run();                               // start and wait
result<string> output();  task<result<string>> async_output();                     // run with the standard output captured; the standard error into captured_err when err is null
result<string> combined_output();  task<result<string>> async_combined_output();   // both streams into one string, in the order written
result<tracked_ptr<file>> stdin_pipe();         // a pipe the child reads: the program's end, closed by wait()
result<tracked_ptr<file>> stdout_pipe();        // a pipe the child writes: read to its end before wait(), which closes it
...
build.dir = "/project";
build.out = os::stdout();                              // the program's own descriptors, inherited
build.err = os::stderr();
...
```cpp
task<vector<string>> versions(vector<string> tools) {
    vector<task<result<string>>> asked;
    vector<io::command> commands;
    for (auto& t : tools) {
        commands.push_back(io::command(t, "--version"));
    }
    for (auto& c : commands) {
        asked.push_back(spawn(c.async_output()));   // all at once
    }
    vector<string> out;
    for (auto& a : asked) {
        auto v = co_await a;
...
result<void> signal(int sig);                   // errc::process_done after the wait or the release
result<void> kill();                            // SIGKILL
result<process_state> wait();                   // waitpid, once
task<result<process_state>> async_wait();       // on the reactor: exited(pid), then the status
result<void> release();                         // let go of: nothing more from this object
```
...
```cpp
int pid() const noexcept;
bool exited() const noexcept;                   // ended by exiting, not by a signal
...
string str() const;                             // "exit status 1", "signal: killed"
```

### look_path

```cpp
result<string> look_path(const string& file);   // the name itself when it holds a '/' and is executable, else the first executable of the name in PATH; errc::not_found
```
...
- [file](file.md): `pipe`, the descriptors a child inherits; [stream](stream.md): `buffer`, the readers and writers a child is served through; [os](os.md): `stdin()`, `stdout()`, `stderr()`, `environ()`
- [reactor](../async/reactor.md): `exited(pid)`; [stop_token](../async/stop_token.md); [blocking](../async/blocking.md)
