...
Where the interface hands one back: `Optional<T>` from [Channel](../Async/Channel.md)'s `Receive` and `TryReceive` (`None` once the channel is closed and drained), from `TryDequeue` of [ConcurrentQueue](../Concurrent/ConcurrentQueue.md) and `TryPop` of [ConcurrentStack](../Concurrent/ConcurrentStack.md), from `TryGet` of [ConcurrentDictionary](../Concurrent/ConcurrentDictionary.md) and [ConcurrentSortedDictionary](../Concurrent/ConcurrentSortedDictionary.md), and from `co_await g.Next()` of an [AsyncGenerator](../Async/AsyncGenerator.md) (`None` at the end); `Pair<Key, Value>` is what a dictionary holds and what its initializer list is made of.
