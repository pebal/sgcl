...
```cpp
#include "sgcl/core/weak_map.h"   // or "sgcl/sgcl.h"
...
- Thread safety is that of `std::unordered_map`: concurrent readers, or one writer, with the program's own synchronization ([The rules](README.md#the-rules), 6). The collector clearing a key's cell at the same time is safe.
...
struct const_reference { key_pointer key; const T& value; };   // and a const_iterator
using iterator = /* forward iterator over the live entries */;
using const_iterator = /* the same, the values const */;
```

`reference` is what `*it` returns: the object, held, and the value; `it->key`, `it->value`. A `const_iterator` (from a `const` map) gives `const_reference`, the value `const`; standing on an entry holds its object, which is a write to the iterator, not to the map, so a `const` map is walked as a mutable one is.
...
- [weak_set](weak_set.md): the objects alone; [weak_ptr](weak_ptr.md): the key
...
- README: [Weak containers](README.md#weak-containers) under [Weak pointers](README.md#weak-pointers), [The rules](README.md#the-rules)
