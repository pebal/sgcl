...
```cpp
#include "sgcl/core/sorted_multimap.h"   // or "sgcl/sgcl.h"
...
```cpp
sorted_multimap<string, int> scores = {{"ann", 3}, {"bob", 5}, {"ann", 7}};     // ann 3, ann 7, bob 5
sorted_multimap<int, int, std::greater<int>> desc(std::greater<int>{});
sorted_multimap<string, int> taken = std::move(scores);                         // scores is empty now
```
...
m.insert({{"b", 2}, {"b", 3}});
sorted_multimap<string, int> other = {{"q", 17}};
...
```cpp
sorted_multimap<int, string> m = {{1, "a"}, {1, "b"}, {2, "c"}};
...
```cpp
sorted_multimap<int, int> a = {{1, 1}}, b = {{2, 2}};
auto it = a.begin();
...
```cpp
sorted_multimap<string, int> m = {{"a", 1}, {"a", 2}};
...
`sorted_multimap` carries [mixin::enumerable](mixin/enumerable.md) (`exists`, `count_of`, `find_if`, `for_each` over the pairs; `contains`, `min`, `max` its own), [mixin::equatable](mixin/equatable.md), [mixin::comparable](mixin/comparable.md), the bidirectional category, and [mixin::lookup](mixin/lookup.md): `get` the first value under a key, `values_of` all of them.

```cpp
sorted_multimap<int, string> names = {{1, "a"}, {1, "b"}, {2, "c"}};
...
```cpp
sorted_multimap<int, int> a = {{1, 1}, {1, 2}}, b = {{1, 1}, {1, 3}};
bool less = a < b;                                // true
bool same = a == b;                               // false
```
...
```cpp
sorted_multimap<int, int> m = {{1, 1}, {1, 2}, {2, 3}};
...
```cpp
sorted_multimap m = {std::pair{1, 2.0}, std::pair{1, 3.0}};     // sorted_multimap<int, double>
sorted_multimap from_range(m.begin(), m.end());                 // sorted_multimap<int, double>
```
