...
```cpp
#include "sgcl/core/sorted_set.h"   // or "sgcl/sgcl.h"
...
- Thread safety is that of `std::set`: concurrent readers, or one writer, with the program's own synchronization ([The rules](README.md#the-rules), 6).
...
O(log n), reading raw pointers only. `count` is 0 or 1. The `K` overloads exist for a transparent comparator, which `std::less` of a [string](string.md) is.
...
`sorted_set` carries [mixin::enumerable](mixin/enumerable.md) — `exists`, `all`, `count_of`, `find_if`, `for_each`, `index_of` (the position in the order); `contains` is the set's own, by the key, `min()` and `max()` the ends of the order, O(1) — [mixin::equatable](mixin/equatable.md), [mixin::comparable](mixin/comparable.md) and the bidirectional category ([the mixins](mixin/README.md)); not `mixin::ordered` (the order is the set's, `lower_bound` its own), not `mixin::sequence`.
...
Owns one unlinked node: movable, not copyable; the element is destroyed when the handle dies without having been inserted. The handle holds the node through a `tracked_ptr`, so it lives on a stack or inside a managed object. It is the same handle type as `sgcl::sorted_multiset<Key>::node_type`. There is no `swap` member.
...
set(InputIt, InputIt, Compare = Compare()) -> sorted_set<Key, Compare>;
template<class Key, class Compare = std::less<Key>>
set(std::initializer_list<Key>, Compare = Compare()) -> sorted_set<Key, Compare>;
```
...
- [sorted_multiset](sorted_multiset.md) for equal keys, [sorted_map](sorted_map.md) and [sorted_multimap](sorted_multimap.md) for key-value pairs, [set](set.md) for a hash table
