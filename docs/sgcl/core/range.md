...
The same class in the `Sgcl` interface: [Range](../Sgcl/Core/Range.md).
...
There is no step and no walk downwards: `range(first, last)` with `first > last` is empty, not a descent, and a plain `for` says a step better than a third argument would. `std::views::iota` is the standard's counting range; `sgcl::range` exists so that one class serves both the lookups and the counting, under one name in both interfaces; it lives in `core`, next to the aliases, because it needs nothing from the containers and every module's examples count with it.
...
- [vector](../containers/vector.md), [m_sequence](../containers/m_sequence.md): the containers and the algorithms a counted loop indexes into
