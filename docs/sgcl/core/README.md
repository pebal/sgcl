...
- `atomic<tracked_ptr<T>>` and `atomic_ref<tracked_ptr<T>>`: `load`, `store`, `compare_exchange_weak` and `compare_exchange_strong` with `std::memory_order`, plus `wait`/`notify`; lock-free, the loaded object protected by a hazard pointer for the length of the load. A global shared pointer is a `root_ptr` under an `atomic_ref`. `atomic<string>` is the same over the word of a `string`: a string variable that one thread replaces while others read it, one word, no copy. For any other type `atomic<T>` is `std::atomic<T>` and `atomic_ref<T>` is `std::atomic_ref<T>`, so that a program names one atomic for its flags, counters and pointers.
- `managed_frame`, `frame_ptr<Promise>`, `generator<T>`: coroutines with frames on the managed heap, whose locals and parameters are roots ([managed_frame](coroutine.md), [generator](generator.md)); the tasks and the pool of workers that runs them are the async module's ([Coroutines](../async/README.md#coroutines)).
...
- `weak_map<Key, T>`, `weak_multimap<Key, T>`, `weak_set<Key>`: containers keyed by objects they do not keep alive; an entry dies with its object ([Weak containers](README.md#weak-containers)).
...
- `concurrent::queue<T>`, `concurrent::stack<T>`, `concurrent::sorted_map<Key, T>`, `concurrent::sorted_set<Key>`, `concurrent::map<Key, T>`, `concurrent::set<Key>`: lock-free structures shared by any number of threads, with `push`/`try_pop`/`pop`, and `find`/`insert`/`try_emplace`/`erase` with weakly consistent iteration for the maps and sets; `concurrent::copy_on_write<T>`: a value loaded as an immutable snapshot and replaced whole by a copy and a compare-exchange; `async::channel<T>`: `send`/`receive` with the waiting of both sides, `try_*`, `async_*` for coroutines, `close` ([Lock-free containers](../concurrent/README.md#lock-free-containers)).
...
## Containers
The containers of the standard library with their nodes and buffers on the managed heap and the observers built on them (`weak_map`, `weak_set`, `expiry_queue`): a container lives where a `tracked_ptr` may live, its elements are destroyed exactly when `std` destroys them, and the collector reclaims the memory. Each has a header of its own in `sgcl/core/` (`sgcl/core/vector.h`, `sgcl/core/sorted_map.h`...), and `sgcl/core/core.h` brings them all in; the containers against `std`, in numbers, are on [benchmarks](benchmarks.md). The immutable containers are a module of their own, [immutable](../immutable/README.md).
...
| [rooted](rooted.md) | `rooted.h` | a value with tracked pointers inside kept in a managed object of its own under a root: what an exception object, a `std` container, a global or a platform's closure holds instead of the value; `bad_expected_access` carries its error so |
...
| [atomic](atomic.md) | `atomic.h` | lock-free atomic `tracked_ptr` and `string`: `load`, `store`, compare-exchange, `wait`/`notify`, no ABA; `std::atomic<T>` for every other `T` |
| [atomic_ref](atomic_ref.md) | `atomic_ref.h` | the same operations over a `tracked_ptr` that is not declared atomic: the cell of a `root_ptr`, a member |
...
| [thread](thread.md) | `thread.h` | `std::thread` with the callable and the arguments in a managed node of their own: a `tracked_ptr` captured by value as in a `function`; `sgcl::this_thread` is `std::this_thread` |
| [expected](expected.md) | `expected.h` | `std::expected`'s interface (C++23) over a variant: the value and the error laid out apart |
| [string](string.md) | `string.h` | an immutable string on the managed heap: one word, shared by copying, compared and hashed by its contents, no destructor; `split`, `join`, `trim`, `replace` as Go's `strings` has them; UTF-8, with `runes()`, a `char32_t` as a character and Unicode's case and white space |
| [utf8, unicode, runes](utf8.md) | `utf8.h`, `unicode.h` | the encoding (`decode`, `encode`, `count`, `valid`), the code point's case and white space (`to_lower`, `is_space`, `equal_fold`, from generated tables), the code points of a text as a range of the library |
...
| [duration](duration.md) | `duration.h` | a span of time: nanoseconds in 64 bits, Go's text both ways (`"1h30m"`, `parse`), `seconds()` and the other units, saturated arithmetic; converts from every integral `std::chrono` duration and into `std::chrono::nanoseconds` |
| [managed_frame, frame_ptr](coroutine.md) | `coroutine.h` | coroutine frames on the managed heap, whose parameters, locals and promise are roots while the frame is held; `frame_ptr`, the owner of a coroutine that lives anywhere; the four words of a header in front of the frame, the async module's |
| [generator](generator.md) | `generator.h` | a coroutine that `co_yield`s values, consumed with a range-for or `next()`, its frame managed; runs where it is called, no scheduler |
| [clock](clock.md) | `clock.h` | `clock::now()`, the library's time in one place: the steady clock's, or a test's `manual_clock`'s while one is installed; `time_point` |
...
| [mixin::enumerable](mixin/enumerable.md) | `mixin/enumerable.h` | the questions asked of the elements of any range of the library: `find_if`, `exists`, `count_of`, `contains`, `index_of`, `min`, `max` |
| [mixin::equatable, mixin::comparable](mixin/equatable.md) | `mixin/equatable.h`, `mixin/comparable.h` | `==` and `<=>` between two containers, by the elements; [mixin::comparable](mixin/comparable.md) |
| [mixin::ordered](mixin/ordered.md) | `mixin/ordered.h` | the order of a range: `is_sorted`, `binary_search`, `lower_bound`, `sort`, `sort_by`, `stable_sort` |
| [mixin::sequence](mixin/sequence.md) | `mixin/sequence.h` | the writes over a range: `fill`, `reverse` |
| [mixin::lookup](mixin/lookup.md) | `mixin/lookup.h` | a map by its key: `get`, `try_get`, `value_or`, `contains_key`, `keys`, `values` |
| [mixin::immutable](mixin/immutable.md) | `mixin/immutable.h` | a declaration without methods: a value that never changes, every change a new container; `immutable::vector`, `immutable::list`, `immutable::map`, `immutable::set` |
| [mixin::text](mixin/text.md) | `mixin/text.h` | the read side of `std::string_view` for `string` and `slice<const CharT>` |
| [req](req.md) | `req.h` | the requirements, `namespace req`: `enumerable`, `ordered`, `sequence`, `lookup`, `immutable`, `comparable`, ... — what a parameter asks for; nominal for a container, structural for a value |
...
The questions, the order and the writes of a range (`contains`, `index_of`, `find_if`, `sort`, `reverse`, `min`, `for_each`...) are members of every container that iterates, from the mixins of `core` ([the mixins and the requirements](mixin/README.md)); the maps read by their key through [mixin::lookup](mixin/lookup.md).
...
A module of their own, in the namespace `sgcl::immutable` and the directory `sgcl/immutable/`, with a [README](../immutable/README.md) that is their guide (the model: every operation a new version sharing all but the path it changed; the state of a program as a value): [immutable::vector](../immutable/vector.md), [immutable::list](../immutable/list.md), [immutable::map](../immutable/map.md), [immutable::set](../immutable/set.md), and their [benchmarks](../immutable/benchmarks.md) against immer and `std`.
