...
The collector and the pointers: what every other module is built on and what a program needs before anything else. `#include "sgcl/core/core.h"` brings the module in; the module depends on nothing but the standard library. Its counterpart in the `Sgcl` interface is [`Sgcl/Core/`](../Sgcl/Core/README.md); the index of the whole interface is [`docs/sgcl/`](../README.md).

What the module holds: the pointer the collector follows and the two pointers around it (the one `make_tracked` returns and the one that does not keep its object alive), a root that may live in unmanaged memory, the creation of objects on the managed heap, the collector's own interface (a forced cycle, statistics, the memory limit), the compile-time configuration, the value types that keep a tracked pointer apart from data (`variant`, `any`, `function`, `expected`), `string` (immutable, one word, shared by copying; split, join, trim, replace; `parse` and `to_string`) and `string_view` (a piece of a string that holds the object), `range` (a pair of iterators as a range, and the integers of `range(n)`), and the `std` types under the library's names. The engine under it (the heap, the barrier, the roots, a cycle phase by phase), its diagnostics and its constants have a chapter of their own, [the garbage collector](../../garbage_collector/README.md); the code is in `sgcl/core/detail/`.
...
A piece of a string is a `string_view` that holds the string's object: two words, the word and a range, so `p.view(1, 2)` and every piece of `p.split(',')` is a substring with no copy and no lifetime to watch, kept in a container or a managed object as it is; `std::string_view` remains the borrowed view for what takes one ([string_view](string_view.md)).
...
| [string_view](string_view.md) | `string.h` | a view of a string that holds the string's object: two words, `s.view(pos, n)` a substring with no copy and no lifetime to watch; the pieces of `split` |
