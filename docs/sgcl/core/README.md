...
| [mixin/](mixin/README.md) | `mixin/mixin.h` | the mixins (`m_`) and the concepts (`c_`): what a container declares by its bases and what a parameter asks for; a README of their own |
| [m_enumerable](mixin/m_enumerable.md) | `mixin/m_enumerable.h` | the questions asked of the elements of any range of the library: `find_if`, `exists`, `count_of`, `contains`, `index_of`, `min`, `max` |
| [m_equatable, m_comparable](mixin/m_equatable.md) | `mixin/m_equatable.h`, `mixin/m_comparable.h` | `==` and `<=>` between two containers, by the elements; [m_comparable](mixin/m_comparable.md) |
| [m_ordered](mixin/m_ordered.md) | `mixin/m_ordered.h` | the order of a range: `is_sorted`, `binary_search`, `lower_bound`, `sort`, `sort_by`, `stable_sort` |
| [m_sequence](mixin/m_sequence.md) | `mixin/m_sequence.h` | the writes over a range: `fill`, `reverse` |
| [m_lookup](mixin/m_lookup.md) | `mixin/m_lookup.h` | a map by its key: `get`, `try_get`, `value_or`, `contains_key`, `keys`, `values` |
| [m_text](mixin/m_text.md) | `mixin/m_text.h` | the read side of `std::string_view` for `string` and `slice<const CharT>` |
| [concepts](mixin/concepts.md) | `mixin/concepts.h` | `c_enumerable`, `c_ordered`, `c_sequence`, `c_lookup`, `c_comparable`, ...: nominal for a container, structural for a value; the category markers |
