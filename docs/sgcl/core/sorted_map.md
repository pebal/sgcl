...
```cpp
#include "sgcl/core/sorted_map.h"   // or "sgcl/sgcl.h"
...
`at` throws `out_of_range` when the key is absent; with a transparent comparator it takes a key of another type (a `string_view` for a `string`), as the other lookups do. `operator[]` inserts a value-initialized mapped value for a new key (built in place, through `try_emplace`) and returns a reference to it; for a `tracked_ptr` mapped type that is a null pointer.
...
O(log n), reading raw pointers only. `count` is 0 or 1. The `K` overloads exist for a transparent comparator, which `std::less` of a [string](string.md) is: a literal, a `sgcl::string_view` or a `std::string_view` looks up a `sgcl::string` key without building one.
...
`sorted_map` carries [mixin::enumerable](mixin/enumerable.md) (`exists`, `all`, `count_of`, `find_if`, `for_each`, `index_of` over the pairs; `contains`, `min` and `max` are the map's own, by the key and as the ends of the order), [mixin::equatable](mixin/equatable.md), [mixin::comparable](mixin/comparable.md), the bidirectional category, and [mixin::lookup](mixin/lookup.md): the reads by the key that `std::map` makes a program write by hand.
