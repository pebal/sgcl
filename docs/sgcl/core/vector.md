...
```cpp
#include "sgcl/core/vector.h"   // or "sgcl/sgcl.h"
...
- The cost of `push_back`: a compare of two words of the vector object, a store of the element and a store of the count; the growth is a cold call. The old buffer of a reallocation is collected, not freed at once, so the capacity doubles to halve what waits ([Benchmarks: Containers](benchmarks.md#containers): 2.7 ns per `push_back` against 1.6 ns for `std::vector`).
...
template<std::ranges::input_range R> explicit vector(R&& r);   // from a range of what T is made of: the pieces of a string, a view, another container (not a vector: that is the copy)
...
```cpp
vector<int> zeros(4);                         // 0 0 0 0
vector<string> words(2, "x");            // "x" "x"
vector digits = {1, 2, 3};
vector copy(digits.begin(), digits.end());    // deduced: vector<int>
vector<int> taken = std::move(digits);        // digits is empty now
```
...
`at` throws `out_of_range` for `pos >= size()`; `operator[]` does not check.

```cpp
vector v = {10, 20, 30};
v[1] = 25;
try { v.at(3); } catch (const out_of_range&) { /* 3 >= size() */ }
```
...
`front` and `back` require a non-empty vector. `data()` is the buffer as a plain pointer (null for a vector that has no buffer), valid under the same conditions as any pointer to an element. `as_slice()` is the elements as a [slice](slice.md) that holds the buffer: valid whatever the vector does next — a reallocation leaves the slice on the old buffer, alive and unchanged, not on freed memory — and what a stream reads into (`r->read(v.as_slice())`) or a function takes as its range.

```cpp
vector v = {1, 2, 3};
v.front() = 0;
v.back() = 9;
slice<int> all = v.as_slice();       // {0, 2, 9}, holding the buffer
slice<int> tail = v.as_slice(1);     // {2, 9}
...
`size()` is a word of the vector object; `max_size()` is `PTRDIFF_MAX / sizeof(T)`, and a request above it throws `length_error`.
...
```cpp
vector<tracked_ptr<int>> ptrs;
for (int i : range(1000)) {
    ptrs.push_back(make_tracked<int>(i));     // the buffers outgrown on the way are collected
}
...
Destroys the elements past `count`, or appends value-initialized elements (copies of `value`) up to it, reallocating when the capacity does not suffice. The buffer then grows geometrically, as a `push_back` grows it, so `resize(size() + 1)` repeated reallocates as rarely as pushes do. A constructor that throws destroys the elements appended before it and leaves the size as it was.
...
From [mixin::equatable](mixin/equatable.md) and [mixin::comparable](mixin/comparable.md), for elements that compare. Element-wise, as for `std::vector`: `==` compares sizes first, `<=>` is lexicographical with the synthesized three-way comparison (`<=>` of `T` when it has one, else a `std::weak_ordering` built from `<`), so `<`, `<=`, `>`, `>=` and `!=` follow.
...
vector v(src.begin(), src.end());     // vector<double>
```

### std::erase, std::erase_if

```cpp
template<class T, class U> size_t erase(vector<T>& v, const U& value);
template<class T, class Pred> size_t erase_if(vector<T>& v, Pred pred);
```

Declared in `sgcl` and brought into `std`, as the other containers' are: `std::erase(v, x)` and, found by argument-dependent lookup, `erase(v, x)` alike. Remove every element equal to `value`, or satisfying `pred`, and return how many were removed.

```cpp
vector v = {1, 2, 2, 3, 4};
size_t twos = std::erase(v, 2);                                  // 2; v is 1 3 4
size_t big = erase_if(v, [](int x) { return x > 2; });           // 2; v is 1 (unqualified: sgcl's, by ADL)
```
...
struct Node {
    int value;
    vector<tracked_ptr<Node>> edges;       // a managed buffer of traced pointers: any graph
};
