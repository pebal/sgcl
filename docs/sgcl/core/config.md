...
### SGCL_WORKER_SPIN_US, WorkerSpinMicroseconds

```cpp
#define SGCL_WORKER_SPIN_US 20
static constexpr unsigned WorkerSpinMicroseconds = SGCL_WORKER_SPIN_US;
```
...
### SGCL_BLOCKING_THREADS, BlockingThreads

```cpp
#define SGCL_BLOCKING_THREADS 0
static constexpr unsigned BlockingThreads = SGCL_BLOCKING_THREADS;
```
...
### SGCL_BLOCKING_IDLE_MS, BlockingIdleMilliseconds

```cpp
#define SGCL_BLOCKING_IDLE_MS 10000
static constexpr unsigned BlockingIdleMilliseconds = SGCL_BLOCKING_IDLE_MS;
```

How long an idle thread of the blocking pool waits for a job before it exits, in milliseconds: a program that stopped blocking has no threads for it after that long; `blocking_pool::set_idle_time` changes it at run time.
