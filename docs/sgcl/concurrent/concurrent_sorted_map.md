...
- An element is destroyed by the collector with its node, once nothing holds the node: not at the erase, which other threads may be reading it across. This is the one container of the library whose elements outlive their erasure ([Containers](../containers/README.md#containers)); an element that must be released promptly is held by a `tracked_ptr` whose object does its own cleanup, or watched by an [expiry_queue](../containers/expiry_queue.md).
...
```cpp
concurrent_sorted_map();
explicit concurrent_sorted_map(const Compare& comp);
template<std::input_iterator InputIt> concurrent_sorted_map(InputIt first, InputIt last, const Compare& comp = Compare());
concurrent_sorted_map(std::initializer_list<value_type> ilist, const Compare& comp = Compare());
concurrent_sorted_map(const concurrent_sorted_map&) = delete;
```
...
- [sorted_map](../containers/sorted_map.md), the sequential map with the full `std::map` interface
- [atomic](atomic.md), what every link is; [expiry_queue](../containers/expiry_queue.md) for a cleanup when an erased element's node dies
