# sgcl::intern, sgcl::intern_string
...
template<class K> handle get(const K& value);           // when Hash and KeyEqual are transparent
...
- [concurrent_weak_map](concurrent_weak_map.md), [concurrent_weak_set](concurrent_weak_set.md): the containers keyed by the identity of objects held weakly, whose sweep the pool shares
- [string](../core/string.md): one word, compared by identity first, hashed once; [weak_ptr](../core/weak_ptr.md): the entry
- [concurrent_sorted_set](concurrent_sorted_set.md): the table underneath; [root_ptr](../core/root_ptr.md): how the default pool is held
