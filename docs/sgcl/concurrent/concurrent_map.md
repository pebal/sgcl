...
```cpp
#include "sgcl/concurrent/concurrent_map.h"   // or "sgcl/sgcl.h"
...
The same class in the `Sgcl` interface: [ConcurrentSortedDictionary](../Sgcl/Concurrent/ConcurrentSortedDictionary.md).
...
sgcl::concurrent_map<sgcl::string, int, std::greater<sgcl::string>> by_name_desc;
...
- [map](../containers/map.md), the sequential map with the full `std::map` interface
