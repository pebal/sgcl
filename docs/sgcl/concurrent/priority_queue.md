# sgcl::concurrent::priority_queue

```cpp
#include "sgcl/concurrent/priority_queue.h"   // or "sgcl/sgcl.h"
...
template<std::input_iterator InputIt> concurrent::priority_queue(InputIt first, InputIt last, const Compare& comp = Compare());
concurrent::priority_queue(std::initializer_list<T> ilist, const Compare& comp = Compare());
priority_queue(const concurrent::priority_queue&) = delete;
```

An empty queue, nothing allocated until the first push; the elements of the range or the list pushed one by one.

```cpp
concurrent::priority_queue<int> a;                              // std::less: the smallest first
concurrent::priority_queue<int, std::greater<int>> b;           // the largest first
concurrent::priority_queue<int> c = {3, 1, 2};                  // holds 1, 2, 3
auto shorter = [](const std::string& x, const std::string& y) { return x.size() < y.size(); };
concurrent::priority_queue<std::string, decltype(shorter)> d(shorter);
```
...
concurrent::priority_queue<tracked_ptr<Request>, ByPriority> requests;   // a global: 
...
```cpp
concurrent::priority_queue<int> ties;
ties.push(1);
ties.push(1);
ties.push(0);
...
- [concurrent::queue](queue.md) for the FIFO queue, [concurrent::stack](stack.md) for the LIFO one, [concurrent::bounded_queue](bounded_queue.md) for a ring
- [vector](../core/vector.md), the heap's buffer
- [priority_queue](../core/queue.md), the sequential adapter (the largest first, as `std::priority_queue`)
