...
struct Pile { sgcl::tracked_ptr<Item> head; };     // a plain member, used atomically through atomic_ref
sgcl::tracked_ptr stack = sgcl::make_tracked<Pile>();
...
sgcl::thread producer([&slot] {                      // a reference: the tracked_ptr stays on this frame
...
int main() {
    ...
    for (int t : sgcl::range(4)) {
        workers.emplace_back([&slots, t] {              // a reference: the vector stays in main's frame
            for (size_t i : sgcl::range(slots.size())) {
