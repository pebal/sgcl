...
```cpp
#include "sgcl/containers/unordered_set.h"   // or "sgcl/sgcl.h"
...
- An `unordered_set` holds tracked pointers, so it lives on a stack or inside a managed object: never in `new`/`malloc` memory, a `std` container, a global, a `thread_local` or a plain coroutine frame ([The rules](../core/README.md#the-rules), 1). The same holds for a node handle.
...
- Thread safety is that of `std::unordered_set`: concurrent readers, or one writer, with the program's own synchronization ([The rules](../core/README.md#the-rules), 6).
...
```cpp
sgcl::unordered_set primes = {5, 3, 2, 7, 3};                      // 2 3 5 7, in some order
sgcl::unordered_set<int> sized(100);                                     // 128 buckets
sgcl::vector<sgcl::string> words = {"b", "a"};
sgcl::unordered_set from_range(words.begin(), words.end());              // deduced: unordered_set<sgcl::string>
...
```cpp
sgcl::unordered_set<sgcl::string> s = {"a", "b"};
sgcl::string joined;
...
```cpp
sgcl::unordered_set a = {1, 3};
sgcl::unordered_multiset b = {2, 3, 3};
...
```cpp
sgcl::vector<sgcl::string> src = {"a", "b"};
sgcl::unordered_set from_range(src.begin(), src.end());      // unordered_set<sgcl::string>
...
int main() {
    ...
    sgcl::unordered_set<sgcl::string> names;
