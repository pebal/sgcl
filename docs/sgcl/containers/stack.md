...
```cpp
#include "sgcl/containers/stack.h"   // or "sgcl/sgcl.h"
...
- The container holds tracked pointers, so a stack lives where a `tracked_ptr` may: on a thread's stack or inside a managed object, never in `new`/`malloc` memory, a `std` container, a global or a plain coroutine frame ([The rules](../core/README.md#the-rules), 1).
...
- A `tracked_ptr` may not address an element ([The rules](../core/README.md#the-rules), 4); `top()` is a reference, valid as long as the container's `back()` would be.
...
- [tracked_ptr](../core/tracked_ptr.md), [atomic](../concurrent/atomic.md) for a lock-free stack shared between threads
