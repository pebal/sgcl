# sgcl::unordered_multimap

```cpp
#include "sgcl/containers/unordered_multimap.h"   // or "sgcl/sgcl.h"
...
- An `unordered_multimap` holds tracked pointers, so it lives on a stack or inside a managed object: never in `new`/`malloc` memory, a `std` container, a global, a `thread_local` or a plain coroutine frame ([The rules](../core/README.md#the-rules), 1). The same holds for a node handle.
...
unordered_multimap(InputIt first, InputIt last, size_type bucket_count = 0, const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual());
unordered_multimap(std::initializer_list<value_type> ilist, size_type bucket_count = 0, const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual());
...
```cpp
sgcl::unordered_multimap<sgcl::string, int> scores = {{"ann", 3}, {"bob", 5}, {"ann", 7}};
...
sgcl::unordered_multimap<sgcl::string, int> taken = std::move(scores);              // scores is empty now
```
...
```cpp
sgcl::unordered_multimap<sgcl::string, int> m = {{"a", 1}, {"a", 2}, {"b", 3}};
int total = 0;
...
```cpp
sgcl::unordered_multimap<int, sgcl::string> m = {{1, "a"}, {1, "b"}};
...
```cpp
sgcl::unordered_multimap<sgcl::string, int> m;
m.insert({"a", 1});
auto it = m.insert({"a", 2});                         // in front of the first "a"
m.insert(std::pair<const char*, int>("z", 26));
m.insert({{"b", 2}, {"b", 3}});
sgcl::unordered_multimap<sgcl::string, int> other = {{"q", 17}};
...
```cpp
sgcl::unordered_multimap<sgcl::string, sgcl::string> m;
m.emplace("k", "v");
...
```cpp
sgcl::unordered_multimap<int, sgcl::string> m = {{1, "a"}, {1, "b"}, {2, "c"}};
...
```cpp
void swap(unordered_multimap& other) noexcept(std::is_nothrow_swappable_v<Hash> && std::is_nothrow_swappable_v<KeyEqual>);
...
```cpp
sgcl::unordered_multimap<sgcl::string, int> m = {{"a", 1}, {"a", 2}};   // std::hash and std::equal_to of a string are transparent
...
`unordered_multimap` carries [m_enumerable](../core/mixin/m_enumerable.md) (`contains` its own) and [m_lookup](../core/mixin/m_lookup.md): `get` the first value under a key, `values_of` all of them ([the mixins](../core/mixin/README.md)).

```cpp
sgcl::unordered_multimap<int, int> m = {{1, 10}, {1, 11}};
int sum = 0;
...
Owns one unlinked node: movable, not copyable; the element is destroyed when the handle dies without having been inserted. The handle holds the node through a `tracked_ptr`, so it lives on a stack or inside a managed object. It is the same handle type as `sgcl::unordered_map<Key, T>::node_type`.
...
unordered_multimap(InputIt, InputIt, size_t = 0, Hash = Hash(), KeyEqual = KeyEqual()) -> unordered_multimap<Key, T, Hash, KeyEqual>;
template<class Key, class T, class Hash = std::hash<Key>, class KeyEqual = std::equal_to<Key>>
unordered_multimap(std::initializer_list<std::pair<Key, T>>, size_t = 0, Hash = Hash(), KeyEqual = KeyEqual()) -> unordered_multimap<Key, T, Hash, KeyEqual>;
```
...
sgcl::unordered_multimap from_range(src.begin(), src.end());      // unordered_multimap<sgcl::string, int>
...
int main() {
    ...
    sgcl::unordered_multimap<sgcl::string, sgcl::tracked_ptr<Listener>> topics;
...
- [unordered_map](unordered_map.md) for unique keys, [unordered_multiset](unordered_multiset.md) for keys alone, [multimap](multimap.md) for an ordered tree
