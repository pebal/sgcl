...
```cpp
#include "sgcl/variant.h"   // or "sgcl/sgcl.h"
...
`get` throws `bad_variant_access` (the one of `std`) on the wrong index, `visit` on a valueless variant. `visit` over several variants calls the visitor with one alternative of each; the result type is the visitor's on the first alternatives, which every combination must convert to, or the `R` given.

```cpp
struct Node { int value; };
using Value = sgcl::variant<int, sgcl::tracked_ptr<Node>, std::string>;
...
auto described = sgcl::visit([](const auto& x) -> std::string {
...
- `tests/variant.cpp`: every behaviour above, checked.
