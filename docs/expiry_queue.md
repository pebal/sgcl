...
An empty queue costs nothing beyond an empty `sgcl::vector`. A move hands the entries over; the moved-from queue is empty. The destructor is `clear()`: the entries are dropped without a call, the objects they kept are no longer kept.
