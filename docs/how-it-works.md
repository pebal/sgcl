...
The type's pointer map (`child_pointers.h`) starts full, every word a candidate, and the collector narrows it by elimination: a word found holding a non-zero value that is not a managed address is data, and its offset leaves the map for good. Pointer fields only ever hold null or a managed address, so they never leave.
