...
`get_statistics().phases_ms` (`GetStatistics().PhasesMs`) is the last cycle by phase: registration, states, roots, marking, updated states, sweep, page release, trim (`collector::phase_names`). What to read from it:

- **marking** large and **roots** small: the live heap is large; a full cycle traces all of it. If the program's live set is stable, most cycles should be young ones (`full_cycles` a fraction of `cycles`), whose marking covers the survivors and the dirty pages only ([README: Generations](overview.md#generations)).
- **roots** large: many threads, or deep stacks, or many dirty pages in a young cycle: a program that links old objects more than it allocates stamps a card on every store into an old object, and a dirty page has every marked object on it traced again. `-DSGCL_GENERATIONAL=0` builds a collector of full cycles only, with a barrier that does no carding, for such a program ([config](../sgcl/core/config.md)).
...
- `last_helpers_used` says whether the pool joined: the helpers start from `config::mark_object_threshold` objects and `config::sweep_page_threshold` pages ([config](../sgcl/core/config.md)).

The mutators' side is not in the statistics: it is the write barrier on every pointer copy and the allocation, whose costs the [benchmarks](benchmarks.md#benchmarks) give, and which no diagnostic changes.
