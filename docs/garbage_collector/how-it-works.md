...
- The slow paths, the state store and the card stamp, run in a region: the thread's `in_barrier` word raised, a seq_cst store, before the epoch is read again with a seq_cst load, lowered after the store; the collector, having flipped the epoch, waits for every raised word before it registers (`_wait_barriers`). A Dekker pair with the flip, `stlr` and `ldar` on arm64: either the collector sees the word raised and waits for the stores, or the read sees the new epoch. Without it a thread preempted between its read of the epoch and its store, across the flip, stored a state of the old parity, which no pass takes for a root, or stamped a card the dirty pass had been through (a pointer moved behind the marker, a card stamped late: measured on the stepper's gates). The fast paths store nothing and need nothing.
...
A `unique_ptr` releasing its object to a `tracked_ptr` takes the other path, `store_released`, in two states around the word: first `Releasing`, with the parity the allocation read, a root in either parity as `UniqueLock` is; then the word and the card; then the released state (the parity of the allocation decides between `Reachable` and `Reachable|Fresh`, below), a release store. The object is never a plain word before its state says it has left its `unique_ptr`: a thread that loads the word through an atomic and copies it finds `Releasing`, not a unique object. And a thread preempted inside the release, its epoch read before the flip, harms nothing: the registration finds the object `Releasing` and queues it as a root, and the state of the old parity it stores later lands on a marked object; a stale state that lands before the registration has the word and the card in front of it, which the registration's fence orders before the marking and the dirty pass (before the second state, an object released across the flip was swept, measured on the stepper's gates).
...
What it costs, against a `tracked_ptr`: a construction takes a cell (a store of null into the block's next word and a bump of the index; the block itself is one managed allocation per line of cells), a destruction gives it back (one store), an access is one indirection more, and a copy is a cell of its own. A store or a read through the cell is the store or the read of a `tracked_ptr`. So the rule of thumb: `tracked_ptr` for the code that keeps its pointers where the collector looks anyway, a node structure and the algorithms over it, a lock-free structure, a hot loop with pointer temporaries, the members of managed objects, coroutines on managed frames; `root_ptr` at the boundary, where rule 1 cannot be kept: a global that holds a managed object, a `std::vector` or `std::map` of managed pointers, a `std::function` whose closure goes to the heap (an [`sgcl::function`](../sgcl/core/function.md) keeps it in a managed object), a plain coroutine, a plugin or a library that keeps managed objects in structures of its own, a handle table for another language. A container that must live in such a place goes into a managed object, held by a `root_ptr`.
...
- The flip retires the states of the previous cycle without a pass, so a cycle begins with one store, and a wait for the barriers begun before it, the time a preempted thread takes to run its few stores.
...
- The stacks are scanned once, while the threads run; a thread that exits waits only for the scan of its own stack. The main thread never unregisters: its exit stops the collector, and the static destructors after it use the library as ever.
